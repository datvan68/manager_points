  ...
  const fetchClassStudents = async (selectedClassId: string, page: number, searchVal: string, append: boolean = false) => {
    ...
    try {
      ...
      setHasMoreStudents(prev => ({
        ...prev,
        [selectedClassId]: typeof totalPages === 'number' ? page < totalPages : newStudents.length >= limit,
      }));
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-2.5 sm:gap-3.5 mx-auto w-full flex-1 min-h-0">
        ...
        {isLoadingData ? (
          <div className="bg-white/45 backdrop-blur-md border border-white/70 rounded-2xl p-10 shadow-xs flex flex-col items-center justify-center flex-1 min-h-[250px] gap-3">
          ...
          <form onSubmit={handleSave} className="flex flex-col flex-1 min-h-0">
            ...
            <div className="flex-1 min-h-0 overflow-y-auto pr-0.5 sm:pr-1 custom-scrollbar">
              ...
              <div className="grid grid-cols-12 gap-3.5 sm:gap-4 lg:gap-5 w-full relative z-10 md:h-full md:min-h-0 pb-1">
