...
import { ApplicantProfileFields, compactApplicantProfile, emptyApplicantProfile, PublicDormitoryRegistrationModal, publicRoomTypeForGender } from './PublicDormitoryRegistrationModal';
...
it('matches the create modal layout and submits the existing public payload', async () => {
  ...
  expect(screen.getByPlaceholderText('Tìm sinh viên hoặc nhập họ tên')).toBeInTheDocument();
  expect(screen.queryByText('Mã sinh viên (nếu có)')).not.toBeInTheDocument();
  expect(screen.queryByText('Thông tin hồ sơ (không bắt buộc)')).not.toBeInTheDocument();
  expect(screen.getByRole('button', { name: 'Hủy' })).toBeInTheDocument();
  expect(screen.getByRole('button', { name: 'Gửi đăng ký' })).toBeEnabled();
  fireEvent.click(screen.getByRole('button', { name: 'Hủy' }));
  expect(onOpenChange).toHaveBeenCalledWith(false);

  fireEvent.change(screen.getByPlaceholderText('Tìm sinh viên hoặc nhập họ tên'), { target: { value: 'Nguyễn Văn A' } });
  ...
  fireEvent.change(screen.getByPlaceholderText('Nhập số điện thoại'), { target: { value: '0912345678' } });
  fireEvent.change(screen.getAllByRole('textbox').find(element => element.tagName === 'TEXTAREA')!, { target: { value: 'Tầng 2' } });
  ...
  await waitFor(() => expect(dormitoryApi.public.register).toHaveBeenCalledWith(expect.objectContaining({
    full_name: 'Nguyễn Văn A', gender: 'Female', phone_number: '0912345678', notes: 'Tầng 2', room_type: 'Thường', qr_room_id: 'room-1',
  })));
  const submittedPayload = vi.mocked(dormitoryApi.public.register).mock.calls[0][0];
  expect(submittedPayload).not.toHaveProperty('student_code');
  expect(submittedPayload).not.toHaveProperty('applicant_profile');
});
...
it('persists populated applicant and parent profile fields while omitting blank fields', () => {
  ...
  expect(compactApplicantProfile(applicant_profile)).toEqual({ ethnicity: 'Kinh', citizen_id_issue_date: '2024-01-02', father: { full_name: 'Nguyễn Văn B' } });
});
