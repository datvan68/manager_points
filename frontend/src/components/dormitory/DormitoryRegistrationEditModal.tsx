...
export default function DormitoryRegistrationEditModal({ open, registration, canEdit = true, onOpenChange, onSuccess, onSubmit, restoreFocusRef, successMessage = 'Đã cập nhật mục Danh sách KTX', requirePersonalDetails = true }: DormitoryRegistrationEditModalProps) {
  ...
  return <Dialog open={open} onOpenChange={close}>
    <DialogContent className="max-h-[calc(100dvh-1rem)] w-[calc(100%-1rem)] max-w-4xl overflow-y-auto overscroll-contain rounded-2xl border border-white/80 bg-gradient-to-br from-[#EBF2FA] to-[#DCE6F1] p-4 shadow-2xl max-lg:[scrollbar-width:none] max-lg:[&::-webkit-scrollbar]:hidden sm:max-h-[90vh] sm:p-6">
      ...
      {!canEdit ? <p className="py-6 text-sm text-slate-600">Bạn không có quyền cập nhật mục Danh sách KTX.</p> : <form onSubmit={submit} className="grid gap-4 py-4 sm:grid-cols-2">
        ...
        <div className="space-y-1.5"><label className="px-1 text-[13px] font-bold text-[#1E293B]">Loại phòng</label><div className="hidden lg:block"><Select value={form.room_type} onValueChange={value => setField('room_type', value as EditForm['room_type'])}><SelectTrigger aria-label="Loại phòng"><SelectValue placeholder="Chọn loại phòng" /></SelectTrigger><SelectContent><SelectItem value="Thường">Thường</SelectItem><SelectItem value="Máy lạnh">Máy lạnh</SelectItem></SelectContent></Select></div><div className="lg:hidden"><DormitoryChoicePopover ariaLabel="Loại phòng" value={form.room_type} options={[{ value: 'Thường', label: 'Thường' }, { value: 'Máy lạnh', label: 'Máy lạnh' }]} onValueChange={value => setField('room_type', value as EditForm['room_type'])} /></div></div>
