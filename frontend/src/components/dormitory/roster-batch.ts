...
export async function runRosterBatches<T, R>(items: T[], size: number, send: (batch: T[], index: number) => Promise<R>, onProgress?: (progress: BatchProgress<R, T>) => void) {
  ...
  for (let offset = 0; offset < frozen.length; offset += size) {
    ...
    onProgress?.({ processed: offset, total: frozen.length, status: 'processing', acknowledged, unsent });
    try {
    ...
    onProgress?.({ processed: offset + batch.length, total: frozen.length, status: offset + batch.length === frozen.length ? 'completed' : 'processing', acknowledged, unsent });
  }
