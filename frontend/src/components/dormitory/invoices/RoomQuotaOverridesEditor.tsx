...
interface RoomQuotaOverridesEditorProps {
  ...
  unit: string;
  defaultQuota: number;
  overrides: RoomQuotaOverride[];
  onChange: (overrides: RoomQuotaOverride[]) => void;
  rooms: Room[];
  disabled?: boolean;
}

export default function RoomQuotaOverridesEditor({
  label,
  unit,
  defaultQuota,
  overrides = [],
  ...
  const handleAddRoom = () => {
    ...
    const nextOverrides: RoomQuotaOverride[] = [
      ...overrides,
      {
        room_id: selectedRoomIdToAdd,
        quota_per_person: defaultQuota,
      },
    ];
  ...
  const handleQuotaChange = (index: number, val: number) => {
    const nextOverrides = overrides.map((item, idx) => {
      if (idx !== index) return item;
      return {
        ...item,
        quota_per_person: isNaN(val) ? 0 : Math.max(0, val),
      };
    });
    onChange(nextOverrides);
  };
  ...
  return (
    <div className="mt-3 space-y-2.5 rounded-xl border border-slate-200/80 bg-white/70 p-3 shadow-2xs">
              ...
              >
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                  <input
                    ...
                    step="any"
                    disabled={disabled}
                    aria-label={`Định mức phòng ${displayName}`}
                    value={item.quota_per_person}
                    onChange={(e) => handleQuotaChange(index, parseFloat(e.target.value))}
                    className="w-20 rounded-md border border-slate-200 px-2 py-1 text-right text-xs font-semibold text-[#1E293B] focus:border-[#1A73E8] focus:outline-none focus:ring-1 focus:ring-[#1A73E8]"
                  />
                  <span className="text-[11px] font-medium text-[#64748B]">{unit}/người</span>
                  ...
                  <button
                    ...
                    title="Xóa định mức riêng của phòng này"
                    aria-label={`Xóa định mức riêng của ${displayName}`}
                  >
                    <Trash2 size={14} />
                  </button>
                </div>
      ...
      <p className="text-[11px] text-[#64748B] flex items-center gap-1 pt-0.5">
        <span>* Các phòng khác sẽ tự động áp dụng định mức chung:</span>
        <strong className="text-[#1E293B]">
          {defaultQuota} {unit}/người
        </strong>
      </p>
    </div>
  );
