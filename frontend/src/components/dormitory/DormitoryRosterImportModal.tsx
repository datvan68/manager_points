...
export default function DormitoryRosterImportModal({ isOpen, onClose, onSuccess, onOperationStateChange }: DormitoryRosterImportModalProps) {
  ...
  const importRows = async () => {
    ...
    try {
      ...
      const batchRun = await runRosterBatches<ParsedDormitoryRosterRow, DormitoryRosterImportResponse>(snapshot, 50, async batch => {
        ...
        const details = groupDormitoryRosterImportResults(parts.flatMap(part => part.results));
        ...
        setOperationProgress({ phase: state.status === 'interrupted' ? 'interrupted' : state.status === 'partial' ? 'partial' : state.status === 'completed' ? 'completed' : 'processing', processed: state.processed, total: state.total, counters: totals, details, unconfirmed: state.unconfirmed.length, unsent: state.unsent.length });
      ...
      setOperationProgress(current => current ? { ...current, details: groupDormitoryRosterImportResults(response.results) } : current);
      ...
      if (response.failed > 0) toast.error(`${response.failed} dòng không thể import. Chi tiết đã hiển thị trong cửa sổ kết quả.`);
  ...
  <Popup isOpen={isOpen && !operationProgress} onClose={close} title="Nhập Danh sách KTX từ Excel" className="max-w-3xl" contentClassName="flex min-h-0 flex-1 flex-col p-0">
    <div className="min-h-0 flex-1 overflow-y-auto p-6">
      {semesterWarning && <p role="alert" className="mb-3 rounded-xl border border-amber-500/20 bg-amber-500/10 p-3 text-xs text-amber-700">{semesterWarning}</p>}
      <div className="space-y-4">
      <div className="flex items-start gap-3 rounded-xl border border-blue-500/20 bg-blue-500/10 p-3 text-xs text-[#1A73E8]"><FileSpreadsheet size={18} className="mt-0.5 shrink-0" aria-hidden="true" /><span>Bốn cột bắt buộc: Họ và tên, Ngày sinh, Giới tính, Số điện thoại. Có thể thêm Mã phòng để tự xếp vào giường trống đầu tiên của phòng đó.</span></div>
      {!result && <>
        <div className="flex items-center justify-between gap-3"><div><h3 className="text-sm font-bold text-[#1E293B]">1. Tải tệp mẫu</h3><p className="text-xs text-[#64748B]">Tối đa 10 MB và 5.000 dòng không rỗng.</p></div><button type="button" onClick={downloadTemplate} className="inline-flex items-center gap-2 rounded-xl border border-white/75 bg-white/40 px-3 py-2 text-xs font-semibold text-[#64748B] transition-all duration-150 hover:bg-white/70"><Download size={15} aria-hidden="true" />Tải tệp mẫu</button></div>
        <div><h3 className="mb-2 text-sm font-bold text-[#1E293B]">2. Chọn tệp</h3><label onDragEnter={handleFileDragEnter} onDragOver={event => { event.preventDefault(); event.dataTransfer.dropEffect = 'copy'; if (!busy) setIsDraggingFile(true); }} onDragLeave={handleFileDragLeave} onDrop={handleFileDrop} className={`flex cursor-pointer flex-col items-center gap-2 rounded-xl border border-dashed px-4 py-7 text-center transition-colors ${isDraggingFile ? 'border-blue-500 bg-blue-500/10 ring-2 ring-blue-500/20' : 'border-white/75 bg-white/40 hover:border-blue-500/40'}`}><Upload size={22} className="text-[#1A73E8]" aria-hidden="true" /><span className="text-xs font-semibold text-[#1E293B]">{isDraggingFile ? 'Thả tệp Excel vào đây' : 'Kéo thả hoặc nhấn để chọn .xlsx, .xls'}</span><span className="text-[11px] text-[#64748B]">{file ? `${file.name} · ${(file.size / 1024).toFixed(1)} KB` : 'Tối đa 10 MB'}</span><input ref={fileInputRef} type="file" accept=".xlsx,.xls" className="hidden" onChange={event => { const selected = event.target.files?.[0]; if (selected) selectFile(selected); }} disabled={busy} /></label></div>
      ...
      {result && <div className="space-y-4">{skippedRows > 0 && <div className="rounded-xl border border-amber-500/20 bg-amber-500/10 px-3 py-2 text-xs leading-5 text-amber-700">Đã bỏ qua {skippedRows} dòng lỗi từ bước kiểm tra tệp.</div>}<div className="grid grid-cols-3 gap-3 text-center"><div className="rounded-xl border border-emerald-500/20 bg-emerald-500/10 p-3"><CheckCircle2 className="mx-auto mb-1 text-emerald-600" size={20} aria-hidden="true" /><strong className="block text-lg text-emerald-700">{result.created}</strong><span className="text-[11px] text-emerald-700">Đã tạo</span></div><div className="rounded-xl border border-amber-500/20 bg-amber-500/10 p-3"><AlertCircle className="mx-auto mb-1 text-amber-600" size={20} aria-hidden="true" /><strong className="block text-lg text-amber-700">{result.duplicated}</strong><span className="text-[11px] text-amber-700">Trùng</span></div><div className="rounded-xl border border-rose-500/20 bg-rose-500/10 p-3"><XCircle className="mx-auto mb-1 text-rose-600" size={20} aria-hidden="true" /><strong className="block text-lg text-rose-700">{result.failed}</strong><span className="text-[11px] text-rose-700">Lỗi</span></div></div><section className="overflow-hidden rounded-xl border border-white/75 bg-white/40"><div className="border-b border-white/75 bg-white/50 px-3 py-2 text-xs font-semibold text-[#1E293B]">Chi tiết theo nhóm</div><div className="max-h-64 overflow-auto">{importResultGroups.map(group => <div key={`${group.status}-${group.reason || ''}`} className="grid grid-cols-[auto_1fr] gap-3 border-b border-white/60 px-3 py-2.5 last:border-b-0"><span className="mt-0.5 whitespace-nowrap rounded-xl bg-slate-500/10 px-2 py-1 text-xs font-bold text-[#64748B]">Dòng {formatDormitoryRosterRowRanges(group.rows)}</span><p className="text-xs text-[#64748B]"><span className="font-semibold text-[#1E293B]">{group.status === 'created' ? 'Đã tạo' : group.status === 'duplicated' ? 'Trùng' : 'Lỗi'}</span>{group.rows.length > 1 ? ` · ${group.rows.length} dòng` : ''}{group.reason ? ` · ${group.reason}` : ''}</p></div>)}</div></section></div>}
      </div>
    </div>
