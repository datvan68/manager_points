  ...
  return <Dialog open={open} onOpenChange={close}>
    ...
    >
      <DialogHeader>
        ...
        <DialogDescription className="text-[#64748B]">Chỉ chọn sinh viên đang học và thuộc một lớp hiện tại. Mục KTX: {registration ? `${registration.roster_entry_code} · ${registration.full_name || 'Chưa có tên'}` : ''}</DialogDescription>
      </DialogHeader>
      <div className="space-y-4 py-2">
        ...
        <div className="overflow-hidden rounded-xl border border-white/75 bg-white/40">
          <div className="hidden grid-cols-[auto_1fr_1fr_auto] gap-3 border-b border-white/75 bg-white/50 px-3 py-2 text-xs font-semibold text-[#64748B] sm:grid"><span /> <span>Mã / Họ tên</span><span>Lớp</span><span>Trạng thái</span></div>
          <div className="max-h-[45vh] overflow-y-auto">
            {loading ? <div className="flex items-center justify-center gap-2 px-4 py-10 text-sm text-[#64748B]"><Loader2 className="h-4 w-4 animate-spin motion-reduce:animate-none" aria-hidden="true" />Đang tải sinh viên…</div> : candidates.length ? <div role="radiogroup" aria-label="Sinh viên đủ điều kiện để liên kết">{candidates.map(candidate => { const className = typeof candidate.class_id === 'object' ? candidate.class_id.class_name : candidate.class_id; const isSelected = selected?._id === candidate._id; return <button type="button" role="radio" aria-checked={isSelected} key={candidate._id} onClick={() => setSelected(candidate)} className={`flex w-full items-start gap-3 border-b border-white/60 px-3 py-3 text-left text-sm last:border-b-0 hover:bg-blue-500/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#1A73E8] sm:grid sm:grid-cols-[auto_1fr_1fr_auto] sm:items-center ${isSelected ? 'bg-blue-500/10' : ''}`}><span className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-xl border sm:mt-0 ${isSelected ? 'border-[#1A73E8] bg-[#1A73E8] text-white' : 'border-white/75 bg-white/50'}`} aria-hidden="true">{isSelected ? <Check className="h-3.5 w-3.5" /> : null}</span><span className="min-w-0 flex-1"><strong className="block break-words text-[#1E293B]">{candidate.student_code}</strong><span className="block break-words text-xs text-[#64748B]">{candidate.full_name}</span><span className="block break-words text-xs text-[#64748B] sm:hidden">Lớp: {className || 'Chưa cập nhật'}</span></span><span className="hidden break-words text-xs text-[#64748B] sm:block">{className || 'Chưa cập nhật'}</span><span className="hidden text-xs font-semibold text-emerald-700 sm:block">Đang học</span></button>; })}</div> : <div className="px-4 py-10 text-center text-sm text-[#64748B]">Không có sinh viên hiện tại phù hợp.</div>}
          </div>
        </div>
