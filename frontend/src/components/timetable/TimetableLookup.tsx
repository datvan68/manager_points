...
export default function TimetableLookup({ refreshKey = 0 }: { refreshKey?: number }) {
  ...
  const searchFor = async (next: TimetableFilters, id: number) => {
    try {
      ...
      setResult(data); setState(data.isEmpty ? 'empty' : 'ready');
    } catch (e: unknown) {
  ...
  return <section className="min-w-0 space-y-4" aria-label="Tra cứu thời khóa biểu">
    <div className="rounded-2xl border border-white/75 bg-white/45 p-4 shadow-sm shadow-slate-300/40 backdrop-blur-md">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div><h2 className="text-lg font-semibold text-[#1E293B]">Tra cứu lịch học</h2><p className="mt-1 max-w-2xl text-sm text-[#64748B]">Chỉ tra cứu dữ liệu đã đồng bộ trong database. Chọn lần lượt niên học, học kỳ và tuần.</p></div>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3">
        <button type="button" onClick={() => void search()} disabled={!filters.year || !filters.semester || !filters.week || !filters.className || !options.availableCoverage?.some((item) => selectionKey(item) === selectionKey(filters)) || busy}
        ...
        {filters.week && filters.className && !options.availableCoverage?.some((item) => selectionKey(item) === selectionKey(filters)) && !busy && <p className="text-sm text-amber-700">Dữ liệu thời khóa biểu cho bộ lọc này chưa được đồng bộ.</p>}
      </div>
    </div>
