...
function CatalogPage({ routeBase = '/dormitory/pdf-template', lockedModuleCode }: PdfTemplateCatalogProps) {
  ...
  const effectiveModuleCode = lockedModuleCode || moduleCode;

  const load = async () => {
    setLoading(true); setError('');
    try {
      const result = await pdfTemplateApi.catalog({ page, pageSize, search: query, moduleCode: effectiveModuleCode, featureCode, configured: configuration, sortBy, sortDirection });
  ...
  useEffect(() => { void load(); }, [page, pageSize, query, effectiveModuleCode, featureCode, configuration, sortBy, sortDirection]);
  useEffect(() => {
    let cancelled = false;
    ...
    (async () => {
      try {
        const firstPage = await pdfTemplateApi.catalog({ page: 1, pageSize: 100, configured: 'false', moduleCode: lockedModuleCode || undefined, sortBy: 'displayName', sortDirection: 'asc' });
        ...
        for (let nextPage = 2; nextPage <= pageCount; nextPage += 1) {
          pages.push(await pdfTemplateApi.catalog({ page: nextPage, pageSize: 100, configured: 'false', moduleCode: lockedModuleCode || undefined, sortBy: 'displayName', sortDirection: 'asc' }));
        }
        const unique = new Map(pages.flatMap((result) => result.items).filter((item) => !item.configured && (!lockedModuleCode || item.moduleCode === lockedModuleCode)).map((item) => [item.templateTypeCode, item]));
  ...
  }, [availableRetry, lockedModuleCode]);
  useEffect(() => {
    const next = new URLSearchParams();
    const pairs: [string, string | number][] = [
      ['page', page],
      ['search', query],
      ...(!lockedModuleCode ? [['moduleCode', moduleCode] as [string, string | number]] : []),
      ['featureCode', featureCode],
      ['configured', configuration],
      ['sortBy', sortBy],
      ['sortDirection', sortDirection]
    ];
    pairs.forEach(([key, value]) => value && value !== 'all' && next.set(String(key), String(value)));
    router.replace(`${pathname}?${next.toString()}`, { scroll: false });
  }, [page, query, moduleCode, featureCode, configuration, sortBy, sortDirection, pathname, router, lockedModuleCode]);
  ...
  const features = useMemo(() => facets.features.length ? facets.features : [...new Set(items.filter((item) => effectiveModuleCode === 'all' || item.moduleCode === effectiveModuleCode).map((item) => item.featureCode))].sort(), [facets.features, items, effectiveModuleCode]);
  ...
  return <main className="min-h-0 flex-1 space-y-6 overflow-y-auto p-6" aria-labelledby="pdf-template-title">
    ...
    <div className={`grid gap-3 rounded-2xl border bg-white/80 p-4 ${lockedModuleCode ? 'md:grid-cols-3' : 'md:grid-cols-4'}`} aria-label="Bộ lọc PDF template">
