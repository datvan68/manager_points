...
export default function PdfTemplateEditor({ metadata, onSaved }: Props) {
  ...
  useEffect(() => {
    let revoked = '';
    pdfTemplateApi.source(metadata.templateTypeCode).then((blob) => { revoked = URL.createObjectURL(blob); setSourceUrl(revoked); }).catch(() => undefined);
    return () => { if (revoked) URL.revokeObjectURL(revoked); };
  }, [metadata.templateTypeCode, metadata.version]);
  ...
  const save = async () => {
    ...
    try { await pdfTemplateApi.validate(metadata.templateTypeCode, layout, source); await pdfTemplateApi.save(metadata.templateTypeCode, metadata.version, layout, source); setMessage('Đã lưu template hiện hành.'); onSaved(); }
