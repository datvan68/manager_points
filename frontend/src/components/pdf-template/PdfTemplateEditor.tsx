...
export default function PdfTemplateEditor({ metadata, onSaved, onDirtyChange }: Props) {
  ...
  const save = async () => {
    if (
      !layout ||
      clientErrors.length ||
      !window.confirm('Lưu template này thành cấu hình hiện hành?')
    ) {
      return;
    }
    if (!canRenderPdfOverlays(sourceState, renderedPage, pageIndex)) {
      return setMessage('Hãy chờ trang PDF render thành công trước khi lưu.');
    }
    setBusy(true);
    setMessage('');
    setConflict(false);
  ...
  if (!layout) {
    return (
      <section
        className="rounded-2xl border border-dashed border-slate-300 p-8 text-center text-sm text-slate-500 bg-white/50"
        aria-label="PDF template editor"
      >
        ...
        <label className="mt-4 inline-flex items-center gap-2 cursor-pointer rounded-xl bg-blue-600 px-4 py-2 font-semibold text-white shadow-sm hover:bg-blue-700 transition-colors">
          ...
          <input
            ...
            className="sr-only"
            onChange={(event) => event.target.files?.[0] && replacement(event.target.files[0])}
          />
        </label>
  ...
  return (
    <section className="flex flex-col h-full min-h-0 space-y-4" aria-label="PDF template editor">
      {/* Top Toolbar */}
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200/80 bg-white/90 p-4 shadow-sm backdrop-blur-md shrink-0">
        <div className="flex flex-wrap items-center gap-3">
          <label className="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-slate-50 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-100 cursor-pointer transition-colors shadow-sm">
            Thay PDF nền
            <input
              type="file"
              accept="application/pdf"
          ...
          <label className="inline-flex items-center gap-1.5 text-xs font-medium text-slate-700">
            ...
            <select
              ...
              className="rounded-lg border border-slate-200 bg-white px-2 py-1 text-xs font-semibold text-slate-800 shadow-sm focus:border-blue-500 focus:outline-none"
            >
          ...
          <label className="inline-flex items-center gap-1 text-xs font-medium text-slate-700 cursor-pointer select-none">
            <input
              type="checkbox"
              checked={snap}
        ...
        <div className="flex items-center gap-2">
          {message && (
            <span role="status" className="text-xs font-medium text-slate-600 max-w-xs truncate mr-2">
              {message}
            </span>
          )}
          <button
            ...
            className="rounded-xl bg-slate-800 px-3.5 py-1.5 text-xs font-bold text-white shadow-sm hover:bg-slate-900 disabled:opacity-50 transition-colors"
          >
            Preview
          </button>
          <button
            ...
            className="rounded-xl bg-blue-600 px-3.5 py-1.5 text-xs font-bold text-white shadow-sm hover:bg-blue-700 disabled:opacity-50 transition-colors"
          >
            Save
          </button>
        </div>
      ...
      {/* Main Workspace */}
      <div className="grid flex-1 min-h-0 grid-cols-1 lg:grid-cols-[1fr_320px] gap-4 items-stretch">
        {/* Canvas Area */}
        <div className="overflow-auto rounded-2xl border border-slate-200 bg-slate-100 p-6 relative flex items-start justify-center min-h-[450px] lg:min-h-0">
          <div
                ...
                .map((item) => {
                  ...
                  return (
                    <div
                      ...
                      className={`absolute select-none text-left overflow-visible focus:outline-none transition-colors ${
                        isSelected
                          ? 'border-2 border-blue-600 bg-blue-500/20 text-blue-900 shadow-md ring-2 ring-blue-400/40'
                    ...
                    >
                      <div className="w-full h-full overflow-hidden truncate px-1 text-[10px] font-mono font-semibold leading-tight flex items-center">
                        {item.fieldKey}
                      </div>
      ...
      {previewUrl && (
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm shrink-0">
          <h2 className="mb-2 text-sm font-bold text-slate-900">Synthetic preview</h2>
          <iframe
