    ...
    <section className="rounded-2xl border border-slate-200 bg-white/70 p-4">
      ...
      <div className="grid gap-2 sm:grid-cols-2">
        {roster.students.map((student) => {
          const attended = Boolean(student.attendance);
          return (
            <article key={student._id} className={`rounded-xl border p-3 ${attended ? 'border-emerald-200 bg-emerald-50/70' : 'border-slate-100 bg-white'}`}>
              <div className="flex items-center justify-between gap-3">
                ...
                <button disabled={attended || pending[student._id]} onClick={() => void onCheckin(student._id)}
