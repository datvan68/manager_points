...
function HighlightRow({ item, compact }: { item: StudentHighlightItem; compact?: boolean }) {
  ...
  const isNewDiscipline = type === 'discipline' && item.followUpStatus === 'new';
  const disciplineSummary = isNewDiscipline
    ? <><span className="mr-1 rounded-full bg-rose-100 px-1.5 py-0.5 text-[9px] font-extrabold text-rose-700">{item.newRecordCount ?? 0} ghi nhận mới</span><span>Điểm trừ mới: <strong className="text-rose-700">{item.newImpactScore ?? 0}</strong> • Tổng học kỳ: <strong className="text-rose-700">{item.recordCount} lượt</strong></span></>
    : <><span className="mr-1 font-extrabold text-rose-700">Chưa xử lý</span><span>Số lượt: <strong className="text-rose-700">{item.recordCount} lần</strong> • Điểm bị trừ: <strong className="text-rose-700">{item.impactScore}</strong></span></>;
...
export default function StudentSpotlightPanel({ metrics, refreshKey = '' }: StudentSpotlightPanelProps) {
  ...
  const categoryConfigs: Array<{ id: CategoryId; label: string; icon: typeof AlertTriangle; color: string; list: StudentHighlightItem[] }> = [
    { id: 'discipline', label: 'Kỷ luật cần xử lý', icon: AlertTriangle, color: 'text-rose-500 bg-rose-500/10 border-rose-500/20', list: categories.discipline.items },
            ...
            <div className="flex items-center gap-3 min-w-0 flex-1">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-[10.5px] text-[#64748B] mt-1 truncate font-medium">
                  ...
                  {category.id === 'discipline' && <>{item.followUpStatus === 'new' ? <><span className="mr-1 rounded-full bg-rose-100 px-1.5 py-0.5 text-[9px] font-extrabold text-rose-700">{item.newRecordCount ?? 0} ghi nhận mới</span> Điểm trừ mới: <strong className="text-rose-700">{item.newImpactScore ?? 0}</strong> • Tổng học kỳ: <strong className="text-rose-700">{item.recordCount} lượt</strong></> : <><span className="mr-1 font-extrabold text-rose-700">Chưa xử lý</span> Số lượt: <strong className="text-rose-700">{item.recordCount} lần</strong> • Điểm bị trừ: <strong className="text-rose-700">{item.impactScore}</strong></>} • Ghi nhận: <span className="text-rose-600">{item.groupedRecords?.map(group => `${group.label} (${group.count})`).join(', ')}</span></>}
                </p>
              </div>
            </div>
