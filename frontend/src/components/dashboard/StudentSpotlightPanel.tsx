...
function HighlightRow({ item, compact }: { item: StudentHighlightItem; compact?: boolean }) {
  ...
  const disciplineSummary = <>{isNewDiscipline && <span className="mr-1 rounded-full bg-rose-100 px-1.5 py-0.5 text-[9px] font-extrabold text-rose-700">Ghi nhận mới</span>}<span>Số lần ghi nhận: <strong className="text-rose-700">{item.recordCount}</strong> • Điểm trừ: <strong className="text-rose-700">{item.impactScore}</strong></span></>;
            ...
            <div className="flex items-center gap-3 min-w-0 flex-1">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-[10.5px] text-[#64748B] mt-1 truncate font-medium">
                  ...
                  {category.id === 'discipline' && <>{item.followUpStatus === 'new' && <span className="mr-1 rounded-full bg-rose-100 px-1.5 py-0.5 text-[9px] font-extrabold text-rose-700">Ghi nhận mới</span>} Số lần ghi nhận: <strong className="text-rose-700">{item.recordCount}</strong> • Điểm trừ: <strong className="text-rose-700">{item.impactScore}</strong></>}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-3 shrink-0">
              {category.id !== 'discipline' && <span className="hidden sm:inline text-[9px] text-[#64748B]">{item.latestRecordAt ? new Date(item.latestRecordAt).toLocaleDateString('vi-VN') : ''}</span>}
