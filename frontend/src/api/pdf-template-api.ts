...
export const pdfTemplateApi = {
  async catalog(query: Record<string, string | number | undefined> = {}) { const params = new URLSearchParams(); Object.entries(query).forEach(([key, value]) => value !== undefined && params.set(key, String(value))); return json<PdfTemplateCatalogResponse>(`${API_BASE}/pdf-templates/catalog?${params.toString()}`); },
  async metadata(code: string) { return json<PdfTemplateMetadata>(`${API_BASE}/pdf-templates/${encodeURIComponent(code)}`); },
  async source(code: string, signal?: AbortSignal): Promise<Blob> { const response = await httpClient(`${API_BASE}/pdf-templates/${encodeURIComponent(code)}/source`, { signal }); if (!response.ok) { await handleResponse<never>(response); } const contentType = response.headers.get('content-type')?.split(';')[0].trim().toLowerCase(); if (contentType !== 'application/pdf') throw new Error('Source response không phải PDF.'); const blob = await response.blob(); if (!blob.size) throw new Error('Source PDF rỗng.'); return blob; },
  ...
  async create(code: string, layout: PdfTemplateLayout, source: File) { const form = new FormData(); form.append('layout', JSON.stringify(layout)); form.append('source', source); return json<any>(`${API_BASE}/pdf-templates/${encodeURIComponent(code)}`, { method: 'POST', body: form }); },
  ...
  async delete(code: string, version: number) { return json<{ deleted: true; templateTypeCode: string; version: number }>(`${API_BASE}/pdf-templates/${encodeURIComponent(code)}?version=${encodeURIComponent(version)}`, { method: 'DELETE' }); },
};
