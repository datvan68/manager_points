...
export default function DormitoryRosterPage() {
  ...
  const [bulkDeleteProgress, setBulkDeleteProgress] = useState<{ processed: number; total: number; status: string; deleted: number; blocked: number; notFound: number; invalid: number } | null>(null);
  const [reconcileOpen, setReconcileOpen] = useState(false); const [reconciling, setReconciling] = useState(false); const [reconcileProgress, setReconcileProgress] = useState<{ scanned: number; linked: number; unlinked: number; conflicts: number; failed: number } | null>(null); const [reconcileSemesters, setReconcileSemesters] = useState<Semester[]>([]); const [reconcileSemesterId, setReconcileSemesterId] = useState('');
  ...
  const removeSelected = async () => {
    ...
    try {
      ...
      const run = await runRosterBatches(frozen, 10, batch => dormitoryApi.roster.bulkDelete(batch), state => {
        const outcomes = state.acknowledged as Array<{ deleted: string[]; blocked: Array<{ id: string }>; not_found: string[]; invalid: string[] }>;
        setBulkDeleteProgress({ processed: state.processed, total: state.total, status: state.status, deleted: outcomes.flatMap(item => item.deleted || []).length, blocked: outcomes.flatMap(item => item.blocked || []).length, notFound: outcomes.flatMap(item => item.not_found || []).length, invalid: outcomes.flatMap(item => item.invalid || []).length });
      });
      if (run.status !== 'completed') { toast.error('Xóa bị gián đoạn; batch đang xử lý và các mục chưa xác nhận vẫn được giữ lại.'); return; }
  ...
  useEffect(() => { if (!reconcileOpen) return; void semesterApi.getSemesters().then(items => { setReconcileSemesters(items); const active = items.filter(item => item.status === 'active'); if (active.length === 1) setReconcileSemesterId(active[0]._id); else setReconcileSemesterId(''); }).catch(() => { setReconcileSemesters([]); setReconcileSemesterId(''); }); }, [reconcileOpen]);
  ...
  return <main className="flex h-full min-h-0 flex-col gap-3 overflow-y-auto bg-transparent p-4 custom-scrollbar max-lg:[scrollbar-width:none] max-lg:[&::-webkit-scrollbar]:hidden sm:p-6">
    ...
    <div className={`flex shrink-0 items-center justify-start gap-1 overflow-x-auto scrollbar-none py-0.5 w-full flex-nowrap ${mobileSearchOpen ? 'hidden lg:flex' : ''}`}>
      ...
      <div className="ml-auto flex items-center gap-2 shrink-0 flex-nowrap">
        ...
        {canUpdate && <Button type="button" variant="outline" aria-label="Đối chiếu liên kết KTX" disabled={reconciling || bulkDeleting} onClick={() => setReconcileOpen(true)} className="h-9 shrink-0 rounded-xl border border-white/80 bg-white/50 px-3 text-xs text-slate-700 hover:bg-white/80">Đối chiếu</Button>}
    ...
    {bulkDeleteProgress && <section aria-live="polite" className="rounded-xl border border-rose-200 bg-rose-50 p-3 text-xs text-rose-900"><div className="flex justify-between font-semibold"><span>Tiến độ xóa</span><span>{bulkDeleteProgress.processed}/{bulkDeleteProgress.total} · {bulkDeleteProgress.total ? Math.floor(bulkDeleteProgress.processed / bulkDeleteProgress.total * 100) : 0}%</span></div><div role="progressbar" aria-valuemin={0} aria-valuemax={bulkDeleteProgress.total} aria-valuenow={bulkDeleteProgress.processed} className="mt-2 h-2 rounded-full bg-rose-100"><div className="h-full rounded-full bg-rose-600" style={{ width: `${bulkDeleteProgress.total ? bulkDeleteProgress.processed / bulkDeleteProgress.total * 100 : 0}%` }} /></div><p className="mt-2">Đã xóa {bulkDeleteProgress.deleted} · Bị chặn {bulkDeleteProgress.blocked} · Không tìm thấy {bulkDeleteProgress.notFound} · Không hợp lệ {bulkDeleteProgress.invalid}</p></section>}
    {reconcileProgress && <section aria-live="polite" className="rounded-xl border border-blue-200 bg-blue-50 p-3 text-xs text-blue-900"><div className="flex justify-between font-semibold"><span>Đối chiếu liên kết</span><span>Đã quét {reconcileProgress.scanned}</span></div><p className="mt-2">Đã liên kết {reconcileProgress.linked} · Chưa liên kết {reconcileProgress.unlinked} · Xung đột {reconcileProgress.conflicts} · Lỗi {reconcileProgress.failed}</p></section>}
    ...
    <Dialog open={reconcileOpen} onOpenChange={open => { if (!reconciling) setReconcileOpen(open); }}><DialogContent><DialogHeader><DialogTitle>Đối chiếu liên kết KTX</DialogTitle></DialogHeader><p className="text-sm text-slate-600">Chọn học kỳ để đối chiếu các mục chưa liên kết hoặc đang xung đột.</p><select aria-label="Học kỳ đối chiếu" value={reconcileSemesterId} onChange={event => setReconcileSemesterId(event.target.value)} disabled={reconciling} className="rounded-xl border border-slate-200 p-2 text-sm"><option value="">Chọn học kỳ</option>{reconcileSemesters.map(item => <option key={item._id} value={item._id}>{item.semester_name}</option>)}</select><DialogFooter><Button variant="outline" disabled={reconciling} onClick={() => setReconcileOpen(false)}>Hủy</Button><Button disabled={reconciling || !reconcileSemesterId} onClick={async () => { setReconciling(true); setReconcileProgress({ scanned: 0, linked: 0, unlinked: 0, conflicts: 0, failed: 0 }); try { let cursor: string | undefined; let hasMore = true; while (hasMore) { const response = await dormitoryApi.roster.reconcile({ semester_id: reconcileSemesterId, after_id: cursor, limit: 100 }); setReconcileProgress(current => ({ scanned: (current?.scanned || 0) + response.scanned, linked: (current?.linked || 0) + response.linked, unlinked: (current?.unlinked || 0) + response.unlinked, conflicts: (current?.conflicts || 0) + response.conflicts, failed: (current?.failed || 0) + response.failed })); cursor = response.next_cursor; hasMore = response.has_more && Boolean(cursor); } await load(true); toast.success('Đã hoàn tất đối chiếu liên kết.'); setReconcileOpen(false); } catch (err: any) { toast.error(err?.message || 'Không thể đối chiếu liên kết.'); } finally { setReconciling(false); } }}>Bắt đầu đối chiếu</Button></DialogFooter></DialogContent></Dialog>
