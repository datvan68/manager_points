...
export default function RegistrationsPage() {
  ...
  const columns: ResponsiveColumn<DormRegistration>[] = [
    ...
    { key: 'actions', header: 'Thao tác', priority: 'action', className: 'text-right', render: (_, r) => <div className="flex justify-end gap-1">{canAssignRoom && <RoomAssignmentPopover row={r} onAssigned={assignment => setRegistrations(current => current.map(item => item._id === r._id ? applyRoomAssignment(item, assignment) : item))} />}{canView && <><button type="button" aria-label={`Xem trước đơn ${studentName(r)}`} title="Xem trước đơn" onClick={() => void loadPdfPreview(r)} className="rounded-xl px-2 py-1 text-xs font-medium text-slate-700 hover:bg-slate-100">Xem trước</button><button type="button" aria-label={`Xuất PDF đơn ${studentName(r)}`} title="Xuất PDF" onClick={() => void downloadPdf(r)} className="rounded-xl px-2 py-1 text-xs font-medium text-emerald-700 hover:bg-emerald-50">Xuất PDF</button></>}{canUpdate && <button aria-label={`Sửa đơn ${studentName(r)}`} title="Sửa" onClick={() => openEdit(r)} className="rounded-xl p-1.5 text-blue-600 hover:bg-blue-50"><Pencil size={16} /></button>}{canDelete && <button aria-label={`Xóa đơn ${studentName(r)}`} title="Xóa" onClick={() => setDeleteRow(r)} className="rounded-xl p-1.5 text-red-600 hover:bg-red-50"><Trash2 size={16} /></button>}</div> },
  ];
