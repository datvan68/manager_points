...
export default function ActivitiesAttendancePage() {
  ...
  }, [search, approval, status, dateRange]);
  ...
  const loadMoreMobile = useCallback(async () => {
    ...
    try {
      const result = await activityAttendanceApi.getAll({ ...query, page: nextPage, limit: 40 });
      const nextItems = result.items || [];
      setItems((current) => [...current, ...nextItems.filter((item) => !current.some((row) => row._id === item._id))]);
      mobilePageRef.current = nextPage;
      mobileHasMoreRef.current = nextItems.length === 40 && mobilePageRef.current * 40 < (result.total || 0);
    } catch {
