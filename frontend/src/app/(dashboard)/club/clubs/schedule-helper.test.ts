...
import { ClubSchedule } from '@/api/club-api';
...
describe('schedule-helper', () => {
  ...
  describe('getClubCardTheme', () => {
    it('should return category if valid, ignoring card_ui configuration', () => {
      ...
      expect(getClubCardTheme({ card_ui: { theme: 'sports' }, category: 'academic' })).toBe('academic');
      expect(getClubCardTheme({ card_ui: { theme: 'default' }, category: 'sports' })).toBe('sports');
    });
  ...
  describe('getClubAccentColor', () => {
    it('should ignore custom accent color in card_ui and return color based on category', () => {
      expect(getClubAccentColor({ card_ui: { accent_color: '#F43F5E' }, category: 'academic' })).toBe('#3B82F6');
    });
