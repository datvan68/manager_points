...
export default function ClubScheduleTimeline({ data }: ClubScheduleTimelineProps) {
  ...
  if (!data || !data.items || data.items.length === 0) {
    return (
      <div className="py-12 text-center text-xs text-slate-400 font-semibold flex flex-col items-center justify-center gap-2">
        ...
        <span>Chưa có lịch sinh hoạt nào được lên kế hoạch</span>
      </div>
    );
  }
  ...
  return (
    <div className="relative border-l-2 border-slate-200/85 ml-6 pl-6 space-y-6">
      {data.viewer_mode === 'student' ? (
        // STUDENT MODE
        (data.items as StudentTimelineItem[]).map((s) => {
          ...
          return (
            <div key={s._id} className="relative group">
              {/* Timeline marker */}
              <div className="absolute -left-[33px] top-1.5 w-4 h-4 rounded-full border-2 border-white bg-blue-500 shadow-md group-hover:scale-110 transition-transform" />

              <div className="backdrop-blur-md bg-white/50 hover:bg-white/80 border border-white/80 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all space-y-3.5">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <span className={`text-[9px] font-black px-2 py-0.5 rounded-full border ${schedConfig.bg} ${schedConfig.text} ${schedConfig.border}`}>
                      {schedConfig.label}
                    </span>
                  </div>
                </div>
        ...
        // STAFF MODE
        (data.items as StaffTimelineItem[]).map((s) => {
          const { dateStr } = formatDateTime(s.start_time);
          ...
          return (
            <div key={s._id} className="relative group">
              ...
              <div className="backdrop-blur-md bg-white/50 hover:bg-white/70 border border-white/80 rounded-2xl p-5 shadow-sm transition-all space-y-3.5">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div>
                    <h4 className="text-xs font-black text-slate-800 tracking-wide">{s.title}</h4>
                    <p className="text-[10px] text-slate-450 font-semibold mt-0.5">{dateStr}</p>
                              ...
                              <div className="flex items-center gap-2.5">
                                ...
                                <div className="min-w-0">
                                  ...
                                  <div className="flex items-center gap-2 mt-0.5">
                                    ...
                                    {rec.check_in_time && (
                                      <span className="text-[9px] text-slate-450 font-semibold flex items-center gap-0.5">
                                        <Clock size={10} />
