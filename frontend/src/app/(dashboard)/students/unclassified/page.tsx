...
export default function UnclassifiedStudentsPage() {
  ...
  return <main className="space-y-6 p-4 md:p-6">
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      {loading ? <div className="p-8 text-center text-gray-400">Đang tải...</div> : items.length === 0 ? <div className="p-8 text-center text-gray-400">Không có đăng ký chưa phân loại</div> : <div className="divide-y divide-gray-100">{items.map(item => <div key={item._id} className="p-4 flex items-start gap-3"><div className="p-2 rounded-lg bg-amber-50 text-amber-600"><Users size={18} /></div><div className="min-w-0"><p className="font-medium text-gray-800">{item.ho_ten}</p><p className="text-sm text-gray-500">{item.so_dien_thoai} · {item.email || 'Chưa có email'}</p><p className="text-xs text-gray-400 mt-1">{item.ma_dk_public} · {item.ma_phong || 'Chưa chọn phòng'} · {item.trang_thai}</p></div></div>)}</div>}
    </div>
  </main>;
}
