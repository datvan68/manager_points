...
vi.mock('@/components/grading/CriteriaModal', () => ({ default: ({ isOpen, onSave, onClose }: any) => isOpen ? <div role="dialog" aria-label="criterion modal"><button onClick={onClose}>Hủy</button><button onClick={() => onSave({ categoryId: 'CAT-1', criterion_code: 'C-2', name: 'Mới', type: 'cong_diem', points: 1, minPoints: 0, maxPoints: 5 })}>save criterion</button></div> : null }));
...
const criteria = [{ _id: 'cri-1', category_id: 'cat-1', criterion_code: 'C-1', criterion_name: 'Tham gia hoạt động', criterion_type: 'cong_diem', score_per_unit: 2, min_score: 0, max_score: 10, is_locked: false }, { _id: 'cri-2', category_id: 'cat-2', criterion_code: 'C-2', criterion_name: 'Điểm học tập', criterion_type: 'ky_luat', score_per_unit: 1, min_score: 0, max_score: 5, is_locked: true }];
