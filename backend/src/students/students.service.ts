...
export class StudentsService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    try {
      ...
      this.logger.error(
        'Lỗi khi tự động đồng bộ tài khoản cho sinh viên cũ:',
        syncErr,
      );
    }
  }
  ...
  private async syncLegacyStudentsAccounts() {
    ...
    const studentEmails = students.map((s) =>
      (s.email || `${s.student_code}@school.edu.vn`).toLowerCase(),
    );
    ...
    const legacyCodeUsers = existingUsers.filter((u) =>
      /^\d+$/.test(u.user_name),
    );
    if (legacyCodeUsers.length > 0) {
      this.logger.log(
        `Phát hiện ${legacyCodeUsers.length} tài khoản cũ có user_name là Mã sinh viên. Bắt đầu tự động cập nhật sang Họ và tên...`,
      );
      ...
      this.logger.log(
        `Đã cập nhật thành công Họ và tên cho ${legacyCodeUsers.length} tài khoản sinh viên cũ.`,
      );
    }
    ...
    this.logger.log(
      `Phát hiện ${legacyStudents.length} sinh viên cũ chưa có tài khoản User. Bắt đầu tự động đồng bộ tài khoản...`,
    );
    ...
    for (const student of legacyStudents) {
      try {
        ...
        this.logger.error(
          `Không thể tự động tạo tài khoản cho sinh viên cũ ${student.full_name} (${student.student_code}):`,
          err,
        );
      }
    }
    ...
    if (successCount > 0) {
      this.logger.log(
        `Đã tự động đồng bộ thành công tài khoản cho ${successCount}/${legacyStudents.length} sinh viên cũ.`,
      );
    }
  }
  ...
  private async generateStudentUser(student: any, plainPasswordDob: string) {
    ...
    if (!existingUser) {
      ...
      this.logger.log(
        `Tự động cấp tài khoản đăng nhập (Chưa active) cho sinh viên: ${student.full_name} (Mã: ${student.student_code})`,
      );
    }
  }
  ...
  async create(createStudentDto: CreateStudentDto): Promise<Student> {
    try {
      ...
      try {
        ...
        this.logger.error(
          'Lỗi khi tự động khởi tạo tài khoản đăng nhập cho sinh viên mới:',
          userErr,
        );
      }
      ...
      try {
        ...
        if (summariesToCreate.length > 0) {
          ...
          this.logger.log(
            `Tự động khởi tạo ${summariesToCreate.length} bảng điểm 0đ cho sinh viên mới: ${createStudentDto.full_name}`,
          );
        }
      } catch (sumErr) {
        this.logger.error(
          'Lỗi khi tự động khởi tạo summaries points cho sinh viên mới:',
          sumErr,
        );
      }
  ...
  async createBulk(createStudentDtos: CreateStudentDto[]) {
    try {
      ...
      try {
        ...
        this.logger.error(
          'Lỗi khi tự động khởi tạo tài khoản đăng nhập hàng loạt cho sinh viên mới:',
          userErr,
        );
      }
      ...
      try {
        ...
        if (summariesToCreate.length > 0) {
          await this.summaryPointModel.insertMany(summariesToCreate);
          this.logger.log(
            `Tự động khởi tạo ${summariesToCreate.length} bảng điểm 0đ cho ${createdStudents.length} sinh viên mới trong bulk import.`,
          );
        }
      } catch (sumErr) {
        this.logger.error(
          'Lỗi khi tự động khởi tạo summaries points cho danh sách sinh viên bulk import:',
          sumErr,
        );
      }
  ...
  async findAll(): Promise<any[]> {
    ...
    return students.map((student) => {
      ...
      (studentObj as any).account_status =
        userStatusMap.get(emailKey) || 'inactive';
      return studentObj;
    });
  }
  ...
  async remove(id: string): Promise<Student> {
    ...
    try {
      ...
      const userDelResult = await this.userModel.deleteOne({
        email: studentEmail,
      });
      if (userDelResult.deletedCount > 0) {
        this.logger.log(
          `Tự động xóa tài khoản User liên kết cho sinh viên đã xóa: ${student.full_name} (${student.student_code})`,
        );
      }
    } catch (userErr) {
      this.logger.error(
        `Lỗi khi tự động xóa tài khoản User cho sinh viên đã bị xóa: ${student.full_name}`,
        userErr,
      );
    }
