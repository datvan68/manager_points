...
export class RestoreTypeRegistry implements OnModuleInit {
  ...
  private parseSchemaToRule(schema: any, source: RestoreTypeRuleSource): RestoreTypeRule {
      ...
      if (st.instance === 'ObjectID' || st.instance === 'ObjectId') {
        ...
        if (caster) {
          if (caster.instance === 'ObjectID' || caster.instance === 'ObjectId') {
            ...
            rule.embeddedArrays![path] = this.parseSchemaToRule(caster.schema, source);
          }
        }
    ...
    if (Object.keys(rule.embeddedArrays || {}).length === 0) delete rule.embeddedArrays;
  ...
  private applyOverrideRules() {
    ...
    for (const [collectionName, overrideRule] of Object.entries(overrides)) {
      const existingRule = this.rules.get(collectionName) || { source: 'override' };
      this.rules.set(collectionName, this.mergeRules(existingRule, overrideRule as RestoreTypeRule));
    }
  }

  private mergeRules(base: RestoreTypeRule, override: RestoreTypeRule): RestoreTypeRule {
    const result: RestoreTypeRule = { ...base, source: override.source || base.source };
    
    if (override.objectIds) {
      result.objectIds = Array.from(new Set([...(base.objectIds || []), ...override.objectIds]));
    }
    if (override.objectIdArrays) {
      result.objectIdArrays = Array.from(new Set([...(base.objectIdArrays || []), ...override.objectIdArrays]));
    }
    if (override.dates) {
      result.dates = Array.from(new Set([...(base.dates || []), ...override.dates]));
    }
    
    if (override.embedded) {
      result.embedded = result.embedded || {};
      for (const [key, rule] of Object.entries(override.embedded)) {
        result.embedded[key] = this.mergeRules(result.embedded[key] || { source: 'override' }, rule);
      }
    }
    ...
    if (override.embeddedArrays) {
      ...
      for (const [key, rule] of Object.entries(override.embeddedArrays)) {
        result.embeddedArrays[key] = this.mergeRules(result.embeddedArrays[key] || { source: 'override' }, rule);
      }
    }
  ...
  public normalizeDocument(collectionName: string, doc: Record<string, any>): Record<string, any> {
  ...
  private applyRuleToDoc(doc: any, rule: RestoreTypeRule, pathContext: string): any {
    ...
    if (rule.objectIds) {
      for (const field of rule.objectIds) {
        if (result[field] !== undefined && result[field] !== null) {
          result[field] = this.tryCastObjectId(result[field], `${pathContext}.${field}`);
        }
      }
    }
      ...
      for (const field of rule.objectIdArrays) {
        if (result[field] !== undefined && result[field] !== null) {
           if (Array.isArray(result[field])) {
              result[field] = result[field].map((item: any, i: number) => this.tryCastObjectId(item, `${pathContext}.${field}[${i}]`));
           }
        }
      }
    }
    ...
    if (rule.embedded) {
      for (const [field, subRule] of Object.entries(rule.embedded)) {
         if (result[field] !== undefined && result[field] !== null && typeof result[field] === 'object') {
            result[field] = this.applyRuleToDoc(result[field], subRule, `${pathContext}.${field}`);
         }
      }
    }
    ...
    if (rule.embeddedArrays) {
       for (const [field, subRule] of Object.entries(rule.embeddedArrays)) {
          if (Array.isArray(result[field])) {
             result[field] = result[field].map((item: any, i: number) => this.applyRuleToDoc(item, subRule, `${pathContext}.${field}[${i}]`));
          }
       }
    }
