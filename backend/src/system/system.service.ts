...
import {
  ...
  SystemPerformanceMetricDocument,
...
export class SystemService {
  ...
  constructor(
    ...
    private systemRequestModel: Model<SystemRequestDocument>,
    @InjectModel(DatabaseBackupJob.name)
    private backupJobModel: Model<DatabaseBackupJobDocument>,
    @InjectModel(DatabaseRestoreJob.name)
    private restoreJobModel: Model<DatabaseRestoreJobDocument>,
    @InjectModel(LoginLog.name) private loginLogModel: Model<LoginLogDocument>,
    @InjectModel(User.name) private userModel: Model<UserDocument>,
    @InjectModel(SystemPerformanceMetric.name)
    private performanceMetricModel: Model<SystemPerformanceMetricDocument>,
  ...
  private maskUri(message: string): string {
    ...
    if (mongoUri && message.includes(mongoUri)) {
      return message.replace(
        new RegExp(this.escapeRegExp(mongoUri), 'g'),
        '***MONGO_URI_REDACTED***',
      );
    }
    return message.replace(
  ...
  async getRequestById(id: string) {
    ...
    const request = await this.systemRequestModel
  ...
  async updateRequestStatus(
    id: string,
    dto: UpdateSystemRequestStatusDto,
    userId: string,
    userRole: string,
  ) {
    const request = await this.getRequestById(id);
    ...
    if (terminalStates.includes(currentStatus) && userRole !== 'Admin') {
      throw new BadRequestException(
        'Chỉ có Quản trị viên mới được mở lại yêu cầu đã kết thúc.',
      );
    }
    ...
    await request.save();
    this.logger.log(
      `AUDIT: User ${userId} updated status of request ${id} from ${currentStatus} to ${nextStatus}. Note: ${dto.decision_note}`,
    );
    return request;
  }
  ...
  async getSystemActivity() {
    ...
    const activeBackups = await this.backupJobModel
      .find({ status: { $in: ['queued', 'running'] } })
      .exec();
    const activeRestores = await this.restoreJobModel
      .find({ status: { $in: ['queued', 'running'] } })
      .exec();
    ...
    const checkJob = (job: any, type: string) => {
      const isStaleQueued =
        job.status === 'queued' && job.createdAt < staleQueuedThreshold;
      const isStaleRunning =
        job.status === 'running' && job.createdAt < staleRunningThreshold;
  ...
  async markJobFailed(id: string) {
    ...
    throw new NotFoundException(
      'Không tìm thấy tiến trình đang chạy với ID này.',
    );
  }
  ...
  async getBackupById(id: string) {
    ...
    const job = await this.backupJobModel
      .findById(id)
      .select('-file_path')
      .populate('requested_by', 'user_name email')
      .exec();
  ...
  async createBackup(userId: string, format?: 'auto' | 'archive' | 'ndjson') {
    ...
    if (activity.hasActiveBackup || activity.hasActiveRestore) {
      throw new ConflictException({
        message:
          'Hiện tại đang có tiến trình sao lưu hoặc khôi phục khác đang chạy.',
    ...
    this.logger.log(
      `AUDIT: User ${userId} requested backup creation. Job ID: ${job._id}, format: ${format || 'auto'}`,
    );
  ...
  private async runBackupAsync(
    ...
    try {
      ...
      const collectionsObj = await this.connection.db
        .listCollections()
        .toArray();
      const collections = collectionsObj
        .map((c) => c.name)
      ...
      if (format === 'ndjson') {
        ...
        this.logger.log(
          `Using NDJSON format as requested for backup job ${jobId}`,
        );
        ...
        this.logger.log(
          `Starting mongodump (archive mode) for backup job ${jobId}`,
        );
        ...
        try {
          ...
          this.logger.warn(
            `mongodump failed, trying fallback mongoose stream: ${maskedMsg}`,
          );
          // Fallback to Mongoose custom NDJSON stream
          await this.runMongooseBackupFallback(filePath, collections);
          this.logger.log(
            `Fallback mongoose streaming backup completed for job ${jobId}`,
          );
          job.backup_format = 'ndjson_gzip';
        }
      }
      ...
      this.logger.log(
        `AUDIT: Backup job ${jobId} finished successfully. Size: ${stat.size} bytes`,
      );
    } catch (err) {
      const maskedError = this.maskUri(
        err.message || 'Lỗi không xác định xảy ra trong lúc sao lưu',
      );
  ...
  private async runMongooseBackupFallback(
    filePath: string,
    collections: string[],
  ) {
    const backupStream = new DatabaseBackupStream(
      collections,
      this.connection.db,
    );
  ...
  async downloadBackup(id: string, userId: string) {
    ...
    if (job.status !== 'success') {
      throw new BadRequestException(
        'Không thể tải bản sao lưu chưa hoàn thành hoặc bị lỗi.',
      );
    }
    ...
    const safePrefix = absoluteBackupDir.endsWith(path.sep)
      ? absoluteBackupDir
      : absoluteBackupDir + path.sep;
    const relative = path.relative(safePrefix, absoluteFilePath);
    const isSafe =
      relative && !relative.startsWith('..') && !path.isAbsolute(relative);
    if (!isSafe) {
      throw new ForbiddenException(
        'Truy cập tệp tin ngoài thư mục sao lưu bị từ chối',
      );
    }

    this.logger.log(
      `AUDIT: User ${userId} downloaded backup ${id}. File path: ${job.file_path}`,
    );
  ...
  async deleteBackup(id: string, userId: string) {
    ...
    const job = await this.backupJobModel.findById(id).exec();
    if (!job) {
      const rawDoc = await this.connection
        .collection('database_backup_jobs')
        .findOne({ _id: id as any });
      if (rawDoc) {
        throw new ConflictException(
          'Dữ liệu backup job sai BSON type, cần chạy công cụ sửa lỗi trước khi xóa',
        );
      }
      throw new NotFoundException('Yêu cầu sao lưu không tồn tại');
    }
    ...
    if (job.status === 'queued' || job.status === 'running') {
      throw new ConflictException(
        'Không thể xóa backup đang chạy. Vui lòng đợi hoàn tất hoặc dùng thao tác hủy nếu được hỗ trợ.',
      );
    }
    ...
    if (job.file_path) {
      ...
      if (fileExists) {
        try {
          ...
          this.logger.error(
            `Failed to delete backup file: ${job.file_path}`,
            err,
          );
        }
      }
    ...
    this.logger.log(
      `AUDIT: User ${userId} deleted backup ${id}. File path: ${job.file_path}. File exists: ${fileExists}. File deleted: ${fileDeleted}`,
    );
  ...
  async previewBackupImport(file: any, userId: string) {
    if (!file.originalname.match(/\.(gz|archive|zip)$/)) {
      throw new BadRequestException(
        'Định dạng file không được hỗ trợ: Chỉ chấp nhận file .gz, .archive, .zip',
      );
    }
    ...
    const safePrefix = this.importDir.endsWith(path.sep)
      ? this.importDir
      : this.importDir + path.sep;
    ...
    let detectedFormat: 'mongodump_archive' | 'ndjson_gzip' | 'unknown' =
      'unknown';
    ...
    try {
      try {
        ...
        const sampleText = sampleBuffer.toString(
          'utf8',
          0,
          Math.min(sampleBuffer.length, 500),
        );
        ...
        throw new BadRequestException(
          'File không đúng định dạng gzip hoặc bị lỗi giải nén: ' + e.message,
        );
      }
      ...
      if (detectedFormat === 'ndjson_gzip') {
          ...
          for (const line of lines) {
            ...
            try {
              ...
              if (doc.__collection) {
                ...
                collectionMap.set(
                  currentCollection,
                  (collectionMap.get(currentCollection) || 0) + 1,
                );

                // Validate all documents
                try {
                  const normalizedDoc = this.normalizeMongoTypes(
                    currentCollection,
                    doc,
                  );
                  ...
                  if (
                    ...
                    this.restoreTypeRegistry.hasRule(currentCollection)
                  ) {
              ...
              throw new Error(
                `Lỗi parse JSON tại dòng ${lineCount} (collection: ${currentCollection}): ${e.message}`,
              );
            }
          }
        }
        ...
        if (lineBuffer.trim()) {
          ...
          try {
            ...
            throw new Error(
              `Lỗi parse JSON tại dòng cuối cùng (collection: ${currentCollection}): ${e.message}`,
            );
          }
        }
        ...
        if (hasTypeMismatch) {
          throw new BadRequestException(
            'File import có field sai BSON type (VD: _id không phải ObjectId hợp lệ), không thể khôi phục an toàn. Vui lòng kiểm tra lại file import.',
          );
        }
      } else {
      ...
      if (
        maskedMessage.startsWith('Nội dung file không hợp lệ:') ||
      ...
      if (
        maskedMessage.includes('Lỗi xác thực file archive') ||
        ...
        throw new BadRequestException(
          `Nội dung file không hợp lệ: ${maskedMessage}`,
        );
      }

      throw new BadRequestException(
        `Lỗi hệ thống không xác định: ${maskedMessage}`,
      );
    }
  }
  ...
  async restoreBackupImport(dto: RestoreBackupImportDto, userId: string) {
    ...
    if (job.status !== 'preview') {
      throw new ConflictException(
        'Phiên xem trước không hợp lệ hoặc tiến trình khôi phục đã được chạy',
      );
    }
    ...
    job.pre_restore_backup_job_id = preBackupJob._id;
    await job.save();

    // Run actual backup and then restore
    this.runBackupAndRestoreAsync(
      job._id.toString(),
      preBackupJob._id.toString(),
    ).catch((err) => {
    ...
    const requiresRelogin =
      job.collections &&
      job.collections.some((c) =>
        ['users', 'roles', 'permissions', 'refresh_tokens'].includes(c),
      );
  ...
  async cancelBackupPreview(previewSessionId: string) {
    const job = await this.restoreJobModel
      .findOne({ preview_session_id: previewSessionId })
      .exec();
    if (!job) {
    ...
    if (job.status === 'preview') {
      ...
      if (fs.existsSync(filePath)) {
        try {
          ...
          this.logger.error(
            `Failed to delete temp preview file: ${filePath}`,
            err,
          );
        }
      }
  ...
  private async runBackupAndRestoreAsync(
    restoreJobId: string,
    ...
    try {
      ...
      const preBackupJob = await this.backupJobModel
        .findById(preBackupJobId)
        .exec();
      if (preBackupJob?.status !== 'success') {
        throw new Error(
          'Auto pre-restore backup failed: ' + preBackupJob?.error_message,
        );
      }
      ...
      const filePath = path.resolve(
        this.importDir,
        `import_${restoreJob.preview_session_id}_${sanitizedFileName}`,
      );
      const safePrefix = this.importDir.endsWith(path.sep)
        ? this.importDir
        : this.importDir + path.sep;
      ...
      if (!mongoUri) {
        throw new Error(
          'Cấu hình MONGO_URI bị thiếu, không thể khôi phục dữ liệu',
        );
      }
      ...
      if (isArchive) {
        ...
        try {
          ...
          if (execErr.code === 'ENOENT') {
            throw new Error(
              'Lỗi thiếu công cụ hệ thống: Không tìm thấy công cụ mongorestore trên hệ thống để thực hiện khôi phục. Vui lòng cài đặt MongoDB Database Tools.',
            );
          }
          throw execErr;
        }
          ...
          for (const line of lines) {
            ...
            try {
              ...
              if (doc.__collection) {
                ...
                if (
                  restoreJob.collections.includes(currentCollection) &&
                  restoreJob.mode === 'replace_selected_collections'
                ) {
                  try {
      ...
      if (
        restoreJob.mode === 'replace_selected_collections' &&
        restoreJob.collections.some((c: string) =>
          ['users', 'roles', 'permissions', 'refresh_tokens'].includes(c),
        )
      ) {
        try {
          ...
          this.logger.log(
            'AUDIT: Cleared all refresh tokens due to auth-related collection restore.',
          );
        } catch (e) {}
      }
          ...
          await this.restoreJobModel.updateOne(
            { _id: restoreJob._id },
            {
              $set: {
                ...
                finished_at: restoreJob.finished_at,
              },
            },
          );
        } else {
          ...
          await this.restoreJobModel.updateOne(
            { _id: restoreJob._id },
            {
              $set: {
                status: restoreJob.status,
                finished_at: restoreJob.finished_at,
                error_message: restoreJob.error_message,
              },
            },
          );
        } else {
  ...
  private async insertDocsSafe(
    ...
    const requiresObjectId =
      rule && (rule.objectIds?.includes('_id') || rule.source === 'schema');

    if (
      collectionName === 'database_backup_jobs' ||
      collectionName === 'database_restore_jobs'
    ) {
      for (const doc of docs) {
        if (doc.status === 'queued' || doc.status === 'running') {
          doc.status = 'failed';
          doc.error_message =
            'Imported historical job was not active in this environment';
        }
      }
    }
    ...
    try {
      if (mode === 'merge_upsert') {
        const operations = docs.map((doc) => {
          ...
          if (requiresObjectId && typeof id === 'string') {
            throw new Error(
              `Document in collection ${collectionName} has invalid _id (string) but requires ObjectId: ${id}`,
            );
          }
      ...
      this.logger.error(
        `Error inserting docs into ${collectionName}: ${err.message}`,
      );
      throw new Error(
        `Error inserting docs into ${collectionName}: ${err.message}`,
      );
      ...
      try {
        ...
        if (rule.objectIds) {
          for (const field of rule.objectIds) {
            ...
            orConditions.push({
              [`${field}Type`]: { $nin: ['objectId', 'missing', 'null'] },
            });
          }
        }
        ...
        if (rule.dates) {
          for (const field of rule.dates) {
            projectStage[`${field}Type`] = { $type: `$${field}` };
            orConditions.push({
              [`${field}Type`]: { $nin: ['date', 'missing', 'null'] },
            });
          }
        }
        ...
        if (wrongDocs.length > 0) {
          issues.push({
            ...
            count: wrongDocs.length,
            sample: wrongDocs,
          });
        }
      } catch (e: any) {
        this.logger.error(
          `checkBsonTypes error on ${collectionName}: ${e.message}`,
        );
      }
    }
    ...
    for (const cName of targetCollections) {
      ...
      for (const doc of docs) {
        try {
          ...
          const normalizedDoc = this.restoreTypeRegistry.normalizeDocument(
            cName,
            doc,
          );
          ...
          if (
            cName === 'database_backup_jobs' ||
            cName === 'database_restore_jobs'
          ) {
            if (
              normalizedDoc.file_path &&
              !fs.existsSync(normalizedDoc.file_path)
            ) {
              normalizedDoc.status = 'failed';
              normalizedDoc.error_message =
                'File sao lưu không tồn tại trên máy chủ (Repair BSON)';
            } else if (
              normalizedDoc.status === 'queued' ||
              normalizedDoc.status === 'running'
            ) {
              normalizedDoc.status = 'failed';
              normalizedDoc.error_message =
                'Job chưa hoàn thành từ phiên làm việc trước (Repair BSON)';
            }
          }
          ...
          this.logger.error(
            `Failed to repair document ${doc._id} in ${cName}: ${e.message}`,
          );
          failed++;
        }
      }
  ...
  async checkMongoDbTools(): Promise<{
    mongodump: boolean;
  ...
  private calculateRecommendations(dto: CreateSystemPerformanceMetricDto) {
    ...
    if (dto.load_event_ms && dto.load_event_ms > 3000) {
      recommendations.push({
        ...
        message:
          'Thời gian tải trang vượt 3s. Cần lazy load các tab nặng trong /system.',
      });
    }
    if (dto.lcp_ms && dto.lcp_ms > 2500) {
      recommendations.push({
        severity: 'warning',
        code: 'SYSTEM_LCP_HIGH',
        message:
          'LCP vượt 2.5s. Kiểm tra skeleton/loading state, bundle size và render blocking.',
      });
    }
    if (dto.ttfb_ms && dto.ttfb_ms > 800) {
      recommendations.push({
        severity: 'warning',
        code: 'SYSTEM_TTFB_HIGH',
        message:
          'TTFB vượt 800ms. Kiểm tra backend query, index MongoDB, cache response summary.',
      });
    }
    if (dto.api_total_ms && dto.api_total_ms > 2000) {
      recommendations.push({
        severity: 'warning',
        code: 'SYSTEM_API_TOTAL_HIGH',
        message:
          'Tổng thời gian gọi API vượt 2s. Xác định API chậm nhất từ api_breakdown.',
      });
    }
    if (dto.cls && dto.cls > 0.1) {
      recommendations.push({
        severity: 'info',
        code: 'SYSTEM_CLS_HIGH',
        message:
          'CLS vượt 0.1. Cố định chiều cao table/skeleton, tránh layout shift khi data về.',
      });
    }
    if (dto.inp_ms && dto.inp_ms > 200) {
      recommendations.push({
        severity: 'info',
        code: 'SYSTEM_INP_HIGH',
        message:
          'INP vượt 200ms. Giảm rerender, debounce search/filter, memoize bảng dữ liệu lớn.',
      });
    }
    ...
    if (dto.api_breakdown) {
      for (const api of dto.api_breakdown) {
        if (api.name.includes('login-logs') && api.duration_ms > 1200) {
          recommendations.push({
            severity: 'warning',
            code: 'API_LOGIN_LOGS_SLOW',
            message:
              'API login-logs vượt 1200ms. Xem lại filter, index login_time, action, user_id.',
          });
        }
        if (api.name.includes('requests') && api.duration_ms > 1200) {
          recommendations.push({
            severity: 'warning',
            code: 'API_REQUESTS_SLOW',
            message:
              'API requests vượt 1200ms. Xem lại index status, type, createdAt, populate user.',
          });
        }
        if (api.name.includes('backups') && api.duration_ms > 1200) {
          recommendations.push({
            severity: 'warning',
            code: 'API_BACKUPS_SLOW',
            message:
              'API backups vượt 1200ms. Đảm bảo list backup không expose/scan file path nặng.',
          });
        }
      }
    }
  ...
  async createPerformanceMetric(
    dto: CreateSystemPerformanceMetricDto,
    ...
    const metric = new this.performanceMetricModel({
      ...
      dom_content_loaded_ms: dto.dom_content_loaded_ms
        ? Math.max(0, dto.dom_content_loaded_ms)
        : undefined,
      load_event_ms: dto.load_event_ms
        ? Math.max(0, dto.load_event_ms)
        : undefined,
      ...
      api_total_ms: dto.api_total_ms
        ? Math.max(0, dto.api_total_ms)
        : undefined,
  ...
  async getPerformanceSummary(query: GetPerformanceSummaryQueryDto) {
    ...
    if (!metrics.length) {
      const emptyKeys: Record<string, null> = keys.reduce(
        (acc, k) => ({ ...acc, [k]: null }),
        {},
      );
      return {
    ...
    if (p75.load_event_ms && p75.load_event_ms > 3000) {
      recommendations.push({
        severity: 'warning',
        code: 'SYSTEM_LOAD_P75_HIGH',
        message:
          'Thời gian tải trang (p75) vượt 3s. Đề xuất lazy load các tab nặng trong /system.',
      });
    }
    if (p75.lcp_ms && p75.lcp_ms > 2500) {
      recommendations.push({
        severity: 'warning',
        code: 'SYSTEM_LCP_P75_HIGH',
        message:
          'LCP (p75) vượt 2.5s. Đề xuất giảm bundle/render-blocking, thêm skeleton ổn định.',
      });
    }
    if (p75.ttfb_ms && p75.ttfb_ms > 800) {
      recommendations.push({
        severity: 'warning',
        code: 'SYSTEM_TTFB_P75_HIGH',
        message:
          'TTFB (p75) vượt 800ms. Đề xuất kiểm tra query backend, index MongoDB, cache summary.',
      });
    }
    if (p95.api_total_ms && p95.api_total_ms > 2000) {
      recommendations.push({
        severity: 'warning',
        code: 'SYSTEM_API_TOTAL_P95_HIGH',
        message:
          'Tổng thời gian gọi API (p95) vượt 2s. Đề xuất tìm API chậm nhất từ bảng Slow APIs.',
      });
    }
    if (p75.cls && p75.cls > 0.1) {
      recommendations.push({
        severity: 'info',
        code: 'SYSTEM_CLS_P75_HIGH',
        message:
          'CLS (p75) vượt 0.1. Đề xuất cố định chiều cao table/skeleton.',
      });
    }
    if (p75.inp_ms && p75.inp_ms > 200) {
      recommendations.push({
        severity: 'info',
        code: 'SYSTEM_INP_P75_HIGH',
        message:
          'INP (p75) vượt 200ms. Đề xuất giảm rerender, debounce filter/search, memoize table lớn.',
      });
    }
    ...
    for (const api of slowApis) {
      if (api.name.includes('login-logs') && api.p95 > 1200) {
        recommendations.push({
          ...
          code: 'API_LOGIN_LOGS_SLOW',
          message:
            'API login-logs (p95) vượt 1200ms. Kiểm tra index login_time, action, user_id.',
        });
      }
      if (api.name.includes('requests') && api.p95 > 1200) {
        recommendations.push({
          ...
          code: 'API_REQUESTS_SLOW',
          message:
            'API requests (p95) vượt 1200ms. Kiểm tra index status, type, createdAt, populate user.',
        });
      }
      if (api.name.includes('backups') && api.p95 > 1200) {
        recommendations.push({
          ...
          code: 'API_BACKUPS_SLOW',
          message:
            'API backups (p95) vượt 1200ms. Bảo đảm list backup không scan file system hoặc expose path nặng.',
        });
      }
    }
  ...
  async getDashboardMetrics(requester: any, semesterId?: string) {
    ...
    if (isAdminUser(requester)) {
      ...
      (requester?.permissions || []).some((p: any) =>
        [
          'LOGIN_LOG_READ',
          'SYSTEM_REQUEST_READ',
    ...
    const activeSem =
      ...
      semesters.find((s) => s.status === 'upcoming') ||
      null;
    const targetSemesterId =
      semesterId && Types.ObjectId.isValid(semesterId)
    ...
    if (targetSemesterId) {
      activePeriod = await evaluationPeriodModel
        .findOne({
    ...
    if (roleScope === 'teacher') {
      const teacherClasses = await classModel
        .find({ advisor_id: new Types.ObjectId(requester.userId) })
        .select('_id')
        .lean()
        .exec();
    ...
    if (targetSemesterId) {
      ...
      averageScore =
        avgResult.length > 0 ? Math.round(avgResult[0].avgScore * 10) / 10 : 0;
    }
    ...
    if (roleScope === 'student' && student) {
      ...
      taskFilter.$or = [
        { targetType: 'teacher', targetScope: 'all' },
        {
          targetType: 'teacher',
          targetScope: 'specific',
          targetTeacherIds: new Types.ObjectId(requester.userId),
        },
    ...
    const notifFilter: any = {
      ...
      $or: [
        { recipientUserId: new Types.ObjectId(requester.userId) },
        {
          ...
          targetRole: { $in: ['all', normalizedRoleLower] },
        },
      ],
    };
    ...
    // ─── DISTRIBUTIONS ───────────────────────────────────────────────────────
    // Student Status
    const studentStatusMatch =
      roleScope === 'teacher' ? { class_id: { $in: teacherClassIds } } : {};
    ...
    const classMatch =
      roleScope === 'teacher'
        ? { advisor_id: new Types.ObjectId(requester.userId) }
        : {};
    ...
    const depts = (await departmentModel
      .find({ _id: { $in: deptIds } })
      .select('_id name')
      .lean()
      .exec()) as any[];
    ...
    classDistResult.forEach((item) => {
      const deptName = item._id
        ? deptMap.get(item._id.toString()) || 'Chưa phân khoa'
        : 'Chưa phân khoa';
    ...
    if (targetSemesterId) {
      ...
      scoreDistResult.forEach((item) => {
        if (item._id in scoreDistribution) {
          scoreDistribution[item._id as keyof typeof scoreDistribution] =
            item.count;
        }
      });
    }
    ...
    if (targetSemesterId) {
      if (roleScope === 'teacher') {
        ...
        recentAcademicRecords = await academicRecordModel
          .find(academicRecordFilter)
          .populate('student_id', 'full_name student_code class_id')
          .populate(
            'criterion_id',
            'criterion_name criterion_type score_per_unit',
          )
          .sort({ recorded_at: -1, createdAt: -1 })
          .limit(5)
          .lean()
      ...
      if (roleScope === 'teacher') {
        ...
        topScoresRaw = await summaryPointModel
          .find(topScoresMatch)
          .populate({
            path: 'student_id',
      ...
      const topStudentIds = topScoresRaw
        .map((s: any) => s.student_id?._id)
        .filter(Boolean);
      ...
      const latestRecords = await academicRecordModel
      ...
      const latestRecordMap = new Map<string, any>(
        latestRecords.map((r: any) => [r.student_id.toString(), r]),
      );

      topScores = topScoresRaw.map((s: any) => {
        const latestRecord = latestRecordMap.get(
          s.student_id?._id?.toString() || '',
        );
        ...
        return {
          ...
          latestRecordTitle:
            ...
            'Không có ghi nhận',
          latestRecordAt:
            latestRecord?.recorded_at || latestRecord?.createdAt || null,
          dominantCriterionName:
            latestRecord?.criterion_id?.criterion_name || null,
          type: 'score',
          href: classId
            ? `/students/${classId}/${s.student_id._id}`
            : `/students`,
        };
      });
          ...
          .aggregate([
            ...
            { $sort: { recorded_at: -1, createdAt: -1 } },
            {
              $group: {
                ...
                impactScore: {
                  $sum: {
                    $ifNull: ['$points_effect', '$criterion.score_per_unit'],
                  },
                },
            ...
            { $unwind: { path: '$summary', preserveNullAndEmptyArrays: true } },
            {
              $project: {
                ...
                dominantCriterionName: {
                  $ifNull: ['$latestRecord.criterion.criterion_name', null],
                },
      ...
      if (roleScope === 'student' && studentIds.length > 0) {
        ...
        const disciplineCriteria = await criterionModel
        ...
        if (studentInfo) {
          const classObj = studentInfo.class_id
            ? ((await classModel
                .findById(studentInfo.class_id)
                .select('class_name')
                .lean()
                .exec()) as any)
            : null;
          ...
          const mapRecord = (r: any, defaultType: string) => {
            const pointsEffect =
              r.points_effect !== undefined && r.points_effect !== null
            ...
            return {
              ...
              type: defaultType,
              href: studentInfo.class_id
                ? `/students/${studentInfo.class_id}/${sId}`
                : `/students`,
            };
          };

          const positiveRecords = positiveRecordsRaw.map((r: any) =>
            mapRecord(r, 'cong_diem'),
          );
          const warningRecords = warningRecordsRaw.map((r: any) =>
            mapRecord(r, 'ky_luat'),
          );

          let nextActionLabel = 'Tự đánh giá điểm rèn luyện';
          const nextActionHref = '/grading/score';
    ...
    if (roleScope === 'admin' || roleScope === 'system') {
      ...
      pendingSystemRequests = await requestModel.countDocuments({
        status: 'pending',
        deletedAt: null,
      });
      systemRequestsList = await requestModel
        .find({ deletedAt: null })
        .populate('requester_id', 'user_name email')
        .sort({ createdAt: -1 })
        .limit(5)
      ...
      backupsList = await this.backupJobModel
        .find()
        .populate('requested_by', 'user_name email')
        .sort({ createdAt: -1 })
        .limit(5)
  ...
  private normalizeModuleMaintenanceStates(
    value: any,
  ): Record<string, boolean> {
    const rawStates =
      value?.states && typeof value.states === 'object' ? value.states : value;
    if (!rawStates || typeof rawStates !== 'object' || Array.isArray(rawStates))
      return {};

    return Object.entries(rawStates).reduce<Record<string, boolean>>(
      (acc, [moduleId, isMaintenance]) => {
  ...
  async updateModuleMaintenanceState(
    moduleId: string,
    dto: UpdateModuleMaintenanceDto,
    userId?: string,
  ) {
  ...
  private encrypt(text: string): { iv: string; content: string } {
    ...
    if (!keyString)
      throw new Error(
        'SETTINGS_ENCRYPTION_KEY is missing. Please contact administrator.',
      );
    const key = crypto
      .createHash('sha256')
  ...
  async onModuleInit() {
    try {
      const mailConfig = await this.systemSettingModel
        .findOne({ key: 'MAIL_SMTP_CONFIG' })
        .lean();
      ...
      this.logger.error(
        'Failed to load SMTP config from DB on startup, fallback to ENV.',
        error,
      );
    }
  }
  ...
  async updateMailSettings(dto: UpdateMailSettingsDto) {
    ...
    const currentConfig = await this.systemSettingModel.findOne({
    ...
    await this.systemSettingModel.findOneAndUpdate(
      { key: 'MAIL_SMTP_CONFIG' },
      {
        key: 'MAIL_SMTP_CONFIG',
        value,
        description: 'Cấu hình kết nối MAIL SMTP',
      },
      { upsert: true, returnDocument: 'after' },
    );

    // Apply new config
    const plainPass = passToSave
      ? this.decrypt(passToSave)
  ...
  async testMailConnection(testConfig?: UpdateMailSettingsDto) {
    ...
    if (testConfig) {
      const pass =
        testConfig.pass && testConfig.pass.trim() !== ''
          ? testConfig.pass
          : undefined;
      ...
      if (!passToUse)
        throw new BadRequestException('Mật khẩu SMTP chưa được cấu hình');
