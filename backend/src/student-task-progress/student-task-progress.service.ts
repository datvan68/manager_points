...
import {
  ...
  BulkLinkedTaskProgressEventItemDto,
...
export class StudentTaskProgressService {
  constructor(
    @InjectModel(StudentTaskProgress.name)
    private progressModel: Model<StudentTaskProgressDocument>,
    @InjectModel(StudentTask.name)
    private taskModel: Model<StudentTaskDocument>,
    @InjectModel(Student.name) private studentModel: Model<StudentDocument>,
    @InjectModel(User.name) private userModel: Model<UserDocument>,
    @InjectModel(Role.name) private roleModel: Model<RoleDocument>,
    @InjectModel(Class.name) private classModel: Model<ClassDocument>,
    @InjectModel(SummaryPoint.name)
    private summaryPointModel: Model<SummaryPointDocument>,
  ) {}

  private isSummarySaved(summary: any): boolean {
    if (!summary) return false;
    if (summary.total_score !== null && summary.total_score !== undefined)
      return true;
  ...
  async syncProgressForTask(taskId: string): Promise<{
    ...
    if (!task)
      return { created: 0, reactivated: 0, inactivated: 0, skipped: 0 };
    ...
    if (task.targetType === 'student') {
      ...
      if (task.targetScope === 'all') {
        students = await this.studentModel
          .find({ user_id: { $ne: null }, status: 'Studying' })
          .exec();
      } else {
      ...
      if (task.targetScope === 'all') {
        ...
        if (task.targetTeacherIds && task.targetTeacherIds.length > 0) {
          const users = await this.userModel
            .find({ _id: { $in: task.targetTeacherIds } })
            .exec();
          assigneeUserIds = users.map((u) => ({ userId: u._id }));
        }
      }
      ...
      const supervisorRole = await this.roleModel
        .findOne({ role_code: 'SUPERVISOR' })
        .exec();
      if (supervisorRole) {
        const users = await this.userModel
          .find({ role: supervisorRole._id })
          .exec();
        assigneeUserIds = users.map((u) => ({ userId: u._id }));
      }
    }
    ...
    const existingProgresses = await this.progressModel
      .find({ taskId: task._id })
      .exec();
    const existingProgressMap = new Map(
      existingProgresses.map((p) => [p.assigneeUserId.toString(), p]),
    );
    ...
    const toInactivate = existingProgresses.filter(
      (p) =>
        p.isActive && !validUserIds.some((uid) => uid.equals(p.assigneeUserId)),
    );
  ...
  async getOverview(query: GetProgressOverviewDto, user: any) {
    ...
    const isStudent =
      ...
      roleName.toLowerCase().includes('sinh viên');
    const isTeacher =
      roleName.toLowerCase().includes('teacher') ||
      roleName.toLowerCase().includes('giáo viên') ||
    ...
    if (isStudent) {
      ...
      const teacherTasks = await this.taskModel
        .find(
          {
            deletedAt: null,
            $or: [
    ...
    const summaryResult = await this.progressModel
    ...
    const teacherSummariesRaw = await this.progressModel
      .aggregate(teacherSummaryPipeline)
      .exec();
    const teacherSummaries = teacherSummariesRaw.map((t) => {
      const tr =
        t.totalStudents > 0
          ? Math.round((t.completedStudents / t.totalStudents) * 100)
          : 0;
      let status = 'not_started';
      if (t.totalStudents === 0) status = 'no_data';
      else if (t.completedStudents === t.totalStudents && t.totalStudents > 0)
        status = 'completed';
      else if (t.completedStudents > 0 || t.inProgressStudents > 0)
        status = 'in_progress';
    ...
    const mappedItems = results.map((item) => ({
      ...
      updatedBy: item.updater
        ? { id: item.updater._id.toString(), name: item.updater.user_name }
        : null,
  ...
  async updateStatus(id: string, dto: UpdateProgressStatusDto, user: any) {
    ...
    if (progress.isActive === false) {
      throw new BadRequestException(
        'Không thể cập nhật tiến độ của nhiệm vụ đã bị hủy áp dụng',
      );
    }
    ...
    if (isStudent && progress.assigneeUserId.toString() !== user.userId) {
      throw new ForbiddenException(
        'Bạn chỉ có thể cập nhật tiến độ của chính mình',
      );
    }

    if (
      !isStudent &&
      progress.assigneeUserId.toString() !== user.userId &&
      !hasManagePermission
    ) {
      throw new ForbiddenException(
        'Bạn không có quyền cập nhật tiến độ của người khác',
      );
    }
  ...
  async recalculateTaskAggregateStatus(taskId: string) {
    const activeProgresses = await this.progressModel
      .find({
    ...
    if (activeProgresses.length > 0) {
      const allNotStarted = activeProgresses.every(
        (p) => p.status === StudentTaskStatus.NOT_STARTED,
      );
      const allCompleted = activeProgresses.every(
        (p) => p.status === StudentTaskStatus.COMPLETED,
      );
  ...
  async findProgressByUserAndTasks(
    userId: string,
    taskIds: Types.ObjectId[],
  ): Promise<StudentTaskProgressDocument[]> {
  ...
  async findProgressByUserAndTask(
    userId: string,
    taskId: string,
  ): Promise<StudentTaskProgressDocument | null> {
    return this.progressModel
      .findOne({
        assigneeUserId: new Types.ObjectId(userId),
        taskId: new Types.ObjectId(taskId),
  ...
  async updateProgressFromLinkedEvent(
    dto: LinkedTaskProgressEventDto,
    user: any,
  ): Promise<StudentTaskProgressDocument> {
    ...
    const task = await this.taskModel
      .findOne({ _id: new Types.ObjectId(dto.taskId), deletedAt: null })
      .exec();
    if (!task) {
      throw new NotFoundException(
        'Không tìm thấy nhiệm vụ hoặc nhiệm vụ đã bị xóa',
      );
    }
    ...
    if (!normalizedTaskPage || !AUTO_EVENT_PAGES.includes(normalizedTaskPage)) {
      throw new BadRequestException(
        'Nhiệm vụ này không hỗ trợ tự động đồng bộ tiến độ',
      );
    }
    ...
    if (dto.linkedPage) {
      if (cleanPath(dto.linkedPage) !== normalizedTaskPage) {
        throw new BadRequestException(
          'Trang liên kết không khớp với cấu hình nhiệm vụ',
        );
      }
    }
    ...
    if (task.targetType === 'teacher') {
      if (dto.assigneeStudentId && hasManagePermission && !isStudent) {
        ...
        if (Types.ObjectId.isValid(dto.assigneeStudentId)) {
          studentObj = await this.studentModel
            .findById(dto.assigneeStudentId)
            .select('class_id')
            .lean()
            .exec();
        } else {
          studentObj = await this.studentModel
            .findOne({ student_code: dto.assigneeStudentId })
            .select('class_id')
            .lean()
            .exec();
        }
      ...
      if (dto.assigneeStudentId) {
        ...
        if (Types.ObjectId.isValid(dto.assigneeStudentId)) {
          ...
          if (!student) {
            throw new NotFoundException(
              `Không tìm thấy sinh viên có mã ${dto.assigneeStudentId}`,
            );
          }
          progressQuery.studentId = student._id;
        }
      } else {
        progressQuery.assigneeUserId = new Types.ObjectId(user.userId);
      }
    ...
    if (!progress) {
      throw new ForbiddenException(
        'Bạn không được giao nhiệm vụ này hoặc tiến độ không hoạt động',
      );
    }

    const isUpdatingStudentProgress =
      task.targetType !== 'teacher' && Boolean(dto.assigneeStudentId);

    if (isUpdatingStudentProgress && !hasManagePermission) {
      const isCreator =
        task.createdBy && task.createdBy.toString() === user.userId;
      let isAdvisor = false;
      if (progress.classId) {
        const studentClass = await this.classModel
          .findById(progress.classId)
          .lean()
          .exec();
        if (
          studentClass &&
          studentClass.advisor_id &&
          studentClass.advisor_id.toString() === user.userId
        ) {
          isAdvisor = true;
        }
      }
      if (!isCreator && !isAdvisor) {
        ...
        if (!hasManagePermission) {
          throw new ForbiddenException(
            'Bạn không có quyền cập nhật tiến độ của sinh viên này',
          );
        }
      }
    }
    ...
    if (task.targetType === 'teacher') {
      if (
        dto.sourceId &&
        Types.ObjectId.isValid(dto.sourceId) &&
        dto.sourceType === 'grading_score'
      ) {
        const sourceSummary = await this.summaryPointModel
        ...
        if (sourceSummary && sourceSummary.semester_id) {
          ...
          const advisorClasses = await this.classModel
            .find({ advisor_id: new Types.ObjectId(teacherId) as any })
          ...
          if (classIds.length > 0) {
            const students = await this.studentModel
              .find({ class_id: { $in: classIds as any }, status: 'Studying' })
              .select('_id')
              .exec();
            ...
            const summaries = await this.summaryPointModel
              ...
              .populate('details.criterion_id', 'is_locked')
              .exec();

            let completedStudents = 0;
            const inProgressStudents = 0;
            ...
            const completionRate =
              totalRequiredItems === 0
                ? 0
                : Math.round(
                    (completedTeacherItems / totalRequiredItems) * 100,
                  );
      ...
      if (
        ...
        if (summary) {
          ...
          const statusStr = isSaved ? 'completed' : 'not_started';
        ...
        if (dto.event === 'started' || dto.event === 'completed') {
          ...
          if (!hasManagePermission) {
            throw new ForbiddenException(
              'Bạn không có quyền reset tiến độ nhiệm vụ',
            );
          }
  ...
  async bulkUpdateProgressFromLinkedEvent(
    dto: BulkLinkedTaskProgressEventDto,
    user: any,
      ...
      try {
        ...
        if (item.assigneeStudentId) {
          ...
          if (Types.ObjectId.isValid(item.assigneeStudentId)) {
            progressQuery.studentId = new Types.ObjectId(
              item.assigneeStudentId,
            );
          } else {
            const student = await this.studentModel
              .findOne({ student_code: item.assigneeStudentId })
              .select('_id')
              .lean()
              .exec();
            if (!student) {
              throw new ForbiddenException(
                `Không tìm thấy sinh viên có mã ${item.assigneeStudentId}`,
              );
            }
            progressQuery.studentId = student._id;
          }
        ...
        if (!progress) {
          throw new ForbiddenException(
            `Không tìm thấy tiến độ hợp lệ cho studentId ${item.assigneeStudentId || user.userId}`,
          );
        }
        ...
        if (item.assigneeStudentId && !hasManagePermission) {
          ...
          if (!isCreator && !isAdvisor) {
            if (!hasManagePermission) {
              throw new ForbiddenException(
                `Không có quyền cập nhật tiến độ của sinh viên ${item.assigneeStudentId}`,
              );
            }
          }
        }
        ...
        if (
          item.sourceId &&
          Types.ObjectId.isValid(item.sourceId) &&
          dto.sourceType === 'grading_score'
        ) {
          const summary = await this.summaryPointModel
            .findById(item.sourceId)
            .populate('details.criterion_id', 'is_locked')
            .exec();
          if (summary) {
            const isSaved = this.isSummarySaved(summary);
  ...
  async cascadeStatusToActiveProgresses(
    taskId: string,
    status: string,
    userId: string,
  ...
  async getTeacherProgressDetail(progressId: string, user: any) {
    ...
    if (!hasAdminAccess && user.userId !== teacherId) {
      const isAdvisorOfSomeClasses = await this.classModel.exists({
        advisor_id: new Types.ObjectId(user.userId) as any,
      });
    ...
    if (!hasAdminAccess && user.userId !== teacherId) {
      const userClasses = await this.classModel
        .find({ advisor_id: new Types.ObjectId(user.userId) as any })
        .select('_id')
        .lean()
        .exec();
      const userClassIds = userClasses.map((c) => c._id.toString());
      allowedClassIds = allowedClassIds.filter((id) =>
        userClassIds.includes(id),
      );
      if (allowedClassIds.length === 0) {
        throw new ForbiddenException(
          'Bạn không có quyền xem dữ liệu của các lớp này',
        );
      }
    }

    const filteredClasses = advisorClasses.filter((c) =>
      allowedClassIds.includes(c._id.toString()),
    );
    const classIds = filteredClasses.map((c) => c._id);

    const students = await this.studentModel
      .find({ class_id: { $in: classIds as any }, status: 'Studying' })
      .select('_id student_code full_name class_id')
      .lean()
      .exec();
    ...
    if (
      progress.sourceType === 'grading_score' &&
      progress.sourceId &&
      Types.ObjectId.isValid(progress.sourceId.toString())
    ) {
      const summary = await this.summaryPointModel
        .findById(progress.sourceId)
        .select('semester_id period_id')
        .lean()
        .exec();
      if (summary) {
    ...
    if (!semesterId) {
      ...
      const latestSummary = await this.summaryPointModel
        .findOne(filterLatest)
        .sort({ updatedAt: -1 })
        .select('semester_id period_id')
        .lean()
        .exec();
    ...
    if (semesterId) {
      ...
      summaries = await this.summaryPointModel
        ...
        .populate(
          ...
          'criterion_code criterion_name is_locked',
        )
        .lean()
        .exec();
    }
    ...
    for (const cls of filteredClasses) {
      const classStudents = students.filter(
        (s) => s.class_id?.toString() === cls._id.toString(),
      );
      const studentDetails: any[] = [];

      let classCompletedTeacherItems = 0;
      const classTotalRequiredItems = classStudents.length;
      ...
      for (const student of classStudents) {
        ...
        let status: 'not_started' | 'in_progress' | 'completed' | 'no_data' =
          'no_data';
        ...
        if (summary) {
          ...
          if (summary.details) {
            for (const d of summary.details) {
              const criterion = d.criterion_id;
              const isLocked = criterion?.is_locked === true;
              const countedInProgress = !isLocked;

              const isTeacherHandled =
                (d.gv_score !== null && d.gv_score !== undefined) ||
                (d.final_score !== null && d.final_score !== undefined) ||
                (d.gv_reviewed_at !== null && d.gv_reviewed_at !== undefined);

              let score = null;
              if (d.final_score !== null && d.final_score !== undefined)
                score = d.final_score;
              else if (d.gv_score !== null && d.gv_score !== undefined)
                score = d.gv_score;
              else if (d.sv_score !== null && d.sv_score !== undefined)
                score = d.sv_score;
              else if (d.system_score !== null && d.system_score !== undefined)
              ...
              criteria.push({
                criterionId:
                  ...
                  (typeof d.criterion_id === 'string'
                    ? d.criterion_id
                    : d.criterion_id?.toString()),
      ...
      const classCompletionRate =
        classTotalRequiredItems === 0
          ? 0
          : Math.round(
              (classCompletedTeacherItems / classTotalRequiredItems) * 100,
            );
    ...
    const teacherObj = progress.assigneeUserId
      ? await this.userModel
          .findById(progress.assigneeUserId)
          .select('user_name')
          .lean()
          .exec()
      : null;
    ...
    const overallCompletionRate =
      totalRequiredItemsTotal === 0
        ? 0
        : Math.round(
            (totalCompletedTeacherItems / totalRequiredItemsTotal) * 100,
          );
  ...
  async markAccess(taskId: string, linkedPage?: string, user?: any) {
    ...
    const task = await this.taskModel
      .findOne({ _id: new Types.ObjectId(taskId), deletedAt: null })
      .exec();
    if (!task) {
      throw new NotFoundException(
        'Không tìm thấy nhiệm vụ hoặc nhiệm vụ đã bị xóa',
  ...
  async finalizeExpiredTaskProgress(now: Date = new Date()) {
    const expiredTasks = await this.taskModel
      .find({
        deadline: { $lte: now },
        deletedAt: null,
      })
    ...
    for (const task of expiredTasks) {
      const activeProgresses = await this.progressModel
        .find({ taskId: task._id, isActive: true })
        .exec();
