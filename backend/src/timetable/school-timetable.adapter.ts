...
export class SchoolTimetableAdapter {
  ...
  async getOptions(contextKey: string, filters: Partial<TimetableFilters> = {}): Promise<TimetableOptions> {
    ...
    return this.coalesced(contextKey, key, async () => {
      ...
      const value = await this.lookup(contextKey, filters, (page) => parseTimetableOptions(page.html, filters));
      this.putCache(key, value);
      return value;
    }) as Promise<TimetableOptions>;
  }

  async getTimetable(contextKey: string, filters: TimetableFilters, signal?: AbortSignal): Promise<TimetableResult> {
    ...
    return this.coalesced(contextKey, key, async () => {
      ...
      const value = await this.lookup(contextKey, filters, (page) => parseTimetable(page.html, filters), signal);
      this.putCache(key, value);
      return value;
  ...
  private async loadFreshPage(jar: CookieJar, filters: Partial<TimetableFilters>, signal?: AbortSignal): Promise<TimetableSourcePage> {
    ...
    if (this.isLoginPage(page.html)) {
      ...
      const response = await this.request('./dang-nhap.html', jar, { method: 'POST', body, referer: page.url, signal });
  ...
  private async postback(page: TimetableSourcePage, jar: CookieJar, value: string, field: keyof TimetableFilters, signal?: AbortSignal): Promise<TimetableSourcePage> {
    const $ = cheerio.load(page.html);
    ...
    const next = await this.request($(form).attr('action') || page.url, jar, { method: 'POST', body, referer: page.url, signal });
  ...
  private async submitSearch(page: TimetableSourcePage, jar: CookieJar, filters: TimetableFilters, signal?: AbortSignal): Promise<TimetableSourcePage> {
    const $ = cheerio.load(page.html);
  ...
  private async request(input: string, jar: CookieJar, options: { method?: string; body?: URLSearchParams; referer?: string; redirects?: number; signal?: AbortSignal } = {}): Promise<TimetableSourcePage> {
    ...
    try {
      ...
      if (response.status >= 300 && response.status < 400) {
        ...
        return this.request(location, jar, { referer: url.href, redirects: (options.redirects || 0) + 1, signal: options.signal });
      }
