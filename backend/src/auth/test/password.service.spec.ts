...
describe('PasswordService - OTP Flow', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: getModelToken('PasswordResetRequest'),
          useValue: mockPasswordResetRequestModel,
        },
  ...
  describe('requestPasswordReset (OTP)', () => {
    it('should return neutral response for existing and non-existing emails', async () => {
      ...
      mockPasswordResetRequestModel.updateMany.mockResolvedValue({
        modifiedCount: 0,
      });
      (service as any).passwordResetRequestModel = MockRequestDoc;

      mockUserModel.findOne.mockResolvedValue(null);
      const resNonExist = await service.requestPasswordReset(
        { email: 'nonexist@example.com' },
        '127.0.0.1',
      );
      expect(resNonExist.message).toBe(
        'Nếu email tồn tại, hệ thống đã gửi OTP xác nhận.',
      );
      ...
      const resExist = await service.requestPasswordReset(
        { email: 'exist@example.com' },
        '127.0.0.1',
      );
      expect(resExist.message).toBe(
        'Nếu email tồn tại, hệ thống đã gửi OTP xác nhận.',
      );
    ...
    it('should block if rate limit is exceeded', async () => {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        await service
          .requestPasswordReset({ email: `spam${i}@example.com` }, ip)
          .catch(() => {});
      }

      await expect(service.requestPasswordReset({ email }, ip)).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe('resendPasswordResetOtp', () => {
    it('should block resend if cooldown not expired', async () => {
      ...
      await expect(
        service.resendPasswordResetOtp({
    ...
    it('should create new OTP and invalidate old when resend succeeds', async () => {
      ...
      const res = await service.resendPasswordResetOtp({
  ...
  describe('verifyPasswordResetOtp', () => {
    ...
    it('should block if OTP is incorrect and increment attempts', async () => {
      ...
      await expect(
        service.verifyPasswordResetOtp({ requestId, code: '000000' }),
      ).rejects.toThrow(BadRequestException);
