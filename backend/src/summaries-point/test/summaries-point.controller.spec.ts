...
describe('SummariesPointController', () => {
  ...
  describe('exportSummaryExcel', () => {
    it('should return excel buffer and set headers correctly', async () => {
      ...
      expect(service.generateSummaryExcel).toHaveBeenCalledWith(
        exportDto,
        mockReq.user,
      );
    ...
    it('should return 500 if service throws error', async () => {
      ...
      mockSummariesPointService.generateSummaryExcel.mockRejectedValue(
  ...
  describe('approve', () => {
    it('should call approveGrading on service', async () => {
      ...
      mockSummariesPointService.approveGrading.mockResolvedValue({
        status: 'locked',
        rank_tier: 'diamond',
      });
      ...
      expect(service.approveGrading).toHaveBeenCalledWith(
        'summary1',
        mockReq.user,
      );
  ...
  describe('cancelApproval', () => {
    it('should call cancelApproval on service', async () => {
      const mockReq = { user: { userId: 'admin1', roleName: 'Admin' } };
      mockSummariesPointService.cancelApproval.mockResolvedValue({
        status: 'draft',
        rank_tier: null,
      });
  ...
  describe('cancelApprovalBulk', () => {
    it('should call cancelApprovalBulk on service', async () => {
      ...
      expect(service.cancelApprovalBulk).toHaveBeenCalledWith(
        ['summary1', 'summary2'],
        mockReq.user,
      );
  ...
  describe('getLatest', () => {
    it('should call findLatestForStudent on service', async () => {
      ...
      expect(service.findLatestForStudent).toHaveBeenCalledWith(
        'user1',
        'sem1',
        'period1',
      );
